...

{
	...
	for (i = 0; i < num_data_fields; i++) {
		...
		ptr += sizeof(uint32);
		while (count_received_bytes >= (DISK_BUF_SIZE - bytes_in_buff[index])) {
			copy_bytes = DISK_BUF_SIZE - bytes_in_buff[index];
			memcpy(buffer[index], ptr, copy_bytes);
			count_received_bytes -= copy_bytes;
			pools[curr_read].data_ready = 1;
			pools[curr_read].free = 0;
			ptr += copy_bytes;
			buffer[index] += copy_bytes;
			bytes_in_buff[index] += copy_bytes;
			...
			buffer[index] = pools[curr_read].buffer_ptr[index];
			bytes_in_buff[index] =
	...
	if (CALLBACK_MODE == logging_mode) {
		for (i = 0; i < num_data_fields, bytes_remaining > 0; i++) {
			...
			if (!client_tbl[token].cb_func_ptr) {
				DIAG_LOGE("diag: no callback function registered for proc %d\n", token);
				return;
			}
						...
						uint8 low_wm_val, uint8 high_wm_val)
{
	...
	switch (tx_mode) {
	case DIAG_PERIPH_STREAMING_MODE:
	case DIAG_PERIPH_CIRCULAR_BUFFERING_MODE:
	case DIAG_PERIPH_THRESHOLD_BUFFERING_MODE:
		break;
	default:
	...
	if (peripheral > DIAG_PERIPH_LAST) {
	...
	err = ioctl(fd, DIAG_IOCTL_CONFIG_BUFFERING_TX_MODE, &params, sizeof(params), NULL, 0, NULL, NULL);
	if (err) {
...
int diag_peripheral_buffering_drain_immediate(uint8 peripheral)
{
	...
	err = ioctl(fd, DIAG_IOCTL_BUFFERING_DRAIN_IMMEDIATE, &peripheral,
