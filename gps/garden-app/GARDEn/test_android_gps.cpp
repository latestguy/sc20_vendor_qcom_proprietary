...
void test_gnss_loc_set_system_info_cb (const GnssSystemInfo* info)
{
    garden_print("## test_gnss_loc_set_system_info_cb ##: %d", info->year_of_hw);
}

void test_gnss_loc_sv_status_cb(GnssSvStatus* sv_info)
{

    garden_print("## gnss_sv_status_callback ##:: Number of SVs: %d", sv_info->num_svs);
    if (sOptions.satelliteDetails) {
        for (int i = 0; i<sv_info->num_svs; ++i)
        {
            garden_print("%02d : PRN: %04d, SNR: %09.4f, ELE: %09.4f, AZI: %09.4f"
...
static void test_gps_ni_notify_cb(GpsNiNotification *notification, bool esEnabled)
{
