...
(
    ...
    jboolean hasSpeed,
    jfloat speed,
    jboolean hasNetworkPositionSource,
    jint networkPositionSource
) {
    ALOGV ("%s : %s", "Entering ", __func__);
    ...
    if (osnpGlue) {
        ...
        izatLocation.mHasNetworkPositionSource = hasNetworkPositionSource;
        ...
        izatLocation.mNetworkPositionSource = (IzatNetworkPositionSourceType) networkPositionSource;
...
void onSetRequestJNI (IzatRequest * request) {
    ...
    int providerIndex = static_cast <int> (request->getProvider () >> 1);
    if(providerIndex >=
            (sizeof(providerNameLookUpTable) /
                    sizeof(providerNameLookUpTable[0]))){

        ALOGE ("%s] Invalid provider index %d",
                __func__, providerIndex);
        return;
    }
    ...
    env->NewStringUTF
    (
        providerNameLookUpTable [providerIndex]
    );
...
static JNINativeMethod sMethods[] = {
    ...
    {"nativeOnLocationChanged", "(ZJZJZDZDZFZDZFZFZI)V", (void *) onJavaLocationChanged},
