        ...
        if (inCard->getString("INFO", &info) == 0){
            if (strcmp(info, "REGISTER-EVENT") == 0) {
             ...
             if (strcmp(resp, "LOWI_ASYNC_DISCOVERY_SCAN_RESULTS") == 0) {
                ...
                if(LOWIResponse::SCAN_STATUS_SUCCESS == response->scanStatus) {
                    LOC_LOGV ("WIFI ASYNC DISCOVERY scan results received successfully");
             ...
             if (strcmp(resp, "LOWI_DISCOVERY_SCAN") == 0) {
                ...
                if(LOWIResponse::SCAN_STATUS_SUCCESS == response->scanStatus) {
                    LOC_LOGV ("WIFI DISCOVERY scan results received successfully");

                    mMsgTask->sendMsg (new (nothrow) NotifyLocMsg (this, response));
                }
                else { //retry few times before giving up
                    LOC_LOGV ("NO DISCOVERY SCAN results");
                }
            }
        }
    } while (0);
