...
//                out_file_length(out param): legnth of file
                                               ...
                                               unsigned char* &out_file_content)
{
  ...
  do
  {
    ...
    if (all_file_length > 0)
    {
      ...
      log_error (TAG, "%d, bad file %s", __FUNCTION__, file_name);
      fclose(file_ptr);
      file_ptr =NULL;
      int rc = remove (file_name);
      if(rc != 0)
      {
        log_error (TAG, "%d, failed to remove bad file %s error = %d\n",
                   __FUNCTION__, file_name, errno);
                                  ...
                                  unsigned char* buf, unsigned int length)
{
  ...
  do
  {
    ...
    if (res != 0)
    {
      log_error (TAG, "%s: failed to calcualte EVP digest for %s",
                 __FUNCTION__, file_name);
      result = -1;
      break;
    }
...
//       only if writing to temp file is success and have not currupted
//       content, we will write to original file. if writing to original
//       file is success and have not currupted content, we will delete
//       temp file
                                            ...
                                            const size_t length)
{
  ...
  do
  {
    ...
    {
      file_remove(m_full_path_tmp_file_name_with_extn, temp_file_status);
      log_error(TAG, "%s: saving to temp file currupted",  __FUNCTION__);
      result = -5;
      break;
    }
      ...
      case FS_GOOD:
         switch (temp_file_status)
         {
            case FS_GOOD:
              ...
              // after this eithr original or temporary file will be there
              ...
              {
                // both file still exist? something worng here.
...
// Output:        status, 0 if success
// Description:   check if the file is currupted by comparing
...
bool FaultTolerantFile::check_if_content_is_good(unsigned char *buf, int length)
{
  ...
  if (dig_res != 0)
  {
    log_error (TAG, "%s: failed to calcualte evp digest", __FUNCTION__);
    is_good = false;
  }
...
//                modified_time(out param): last mdified time of file
// Output:        status, 0 if success
...
// Description:   read from where file pointer points to to a buffer
                                               ...
                                               unsigned int &length)
{
  ...
  do
  {
    ...
    if (buf_tmp == NULL)
    {
      log_error(TAG, "%s, allocting memory failed", __FUNCTION__);
      break;
    }
