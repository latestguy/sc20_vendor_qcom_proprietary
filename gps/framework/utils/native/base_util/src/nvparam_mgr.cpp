...
#else
#define NVPARAM_NAME "nvparam.sqlite"
...
private:
  ...
  unsigned long      m_pseudo_client_id_32;
};
...
int NvParamMgrImpl::generatePseudoClientIdIfNeeded ()
{
  ...
  do
  {
    ...
    if (is_valid == true)
    {
      ...
      int rc1 = saveInt32Param(PARAM_NAME_PSEUDO_CLIENT_ID, m_pseudo_client_id_32);
      int rc2 = saveUInt64Param(PARAM_NAME_PSEUDO_CLIENT_ID_64BIT, m_pseudo_client_id_64);
      if (rc1 == 0 || rc2 == 0)
      {
        m_pseudo_client_id_exist = true;
        ret_val = 0;
