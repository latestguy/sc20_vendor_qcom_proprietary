...
public class OsAgent
{
    ...
    private int isDeviceInChina()
    {
        int deviceInChina = mDeviceInChina;
        ...
        if (ServiceState.STATE_IN_SERVICE == mCurrentServiceState) {
            ...
            logv("Before computing is current n/w state china:  " + deviceInChina);
            ...
            switch (phoneType)
            {
                case (TelephonyManager.PHONE_TYPE_CDMA):
                {
                    ...
                    if (iSid == 0) {
                        logd("Sid is zero - Retaining previous n/w status, " + deviceInChina);
                    } else if (((iSid >= CHINA0_SID_MIN && iSid <= CHINA0_SID_MAX) ||
                            (iSid >= CHINA1_SID_MIN && iSid <= CHINA1_SID_MAX) ||
                            (iSid == CHINA_SCOM_SID))) {
                        deviceInChina = DEVICE_IN_REGULATED_REGION_YES;
                    } else {
                        deviceInChina = DEVICE_IN_REGULATED_REGION_NO;
                    }
                    break;
                }
                ...
                case (TelephonyManager.PHONE_TYPE_GSM):
                {
                    ...
                    if (iMcc == 0) {
                        logv("MCC is zero - Retaining previous n/w status " + deviceInChina);
            ...
            logv("After computing is n/w state china, " + deviceInChina);
        } else {
            logv("OOS-last known camped state is China ? " + deviceInChina);
        }

        return deviceInChina;
    }
    ...
    private void handleCellLocationChanged()
    {
        ...
        if (mEnableAutoEula) {
            int isDevInChina = isDeviceInChina();
            if (isDevInChina != mDeviceInChina) {
                boolean regulated = (isDevInChina == DEVICE_IN_REGULATED_REGION_YES);
                mDeviceInChina = isDevInChina;
    ...
    private static final int DEVICE_IN_REGULATED_REGION_YES = 1;
    private static final int DEVICE_IN_REGULATED_REGION_NO = 2;
