...
int get_total_iter_index(int argc, char *argv[], int i)
{
    ...
    switch(cmdId) {
        ...
        case 15:
            ...
            break;
        case 14:
            /* Move i By: "<id> <no_of_bssid> <list_of_bssids>" */
            if (i+1 < argc) {
                bssid_num = atoi(argv[i + 1]);
                if (bssid_num > MAX_BLACKLIST_BSSID) {
                    fprintf(stderr, "%s: Max number of blacklist bssid "
                            "supported is %d\n", __FUNCTION__,
                            MAX_BLACKLIST_BSSID);
                    return 0;
                }
                total_iter_index = i + 2 + bssid_num;
            }
            break;
        default:
