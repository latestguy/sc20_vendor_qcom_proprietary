...
static void sensor_init() {
    ...
    if(!libsnscal) {
        MMI_ALOGE("FFBM SENSOR: 'sensor_calibrate.so' dlopen fail, error=%s\n", MMI_STR(dlerror()));
        return;
    }
    sensor_cal_module = (struct sensor_cal_module_t *) dlsym(libsnscal, SENSOR_CAL_MODULE_INFO_STR);
    if(!sensor_cal_module) {
        MMI_ALOGE("FFBM SENSOR: symbol(%s) loading fail, error=%s\n", SENSOR_CAL_MODULE_INFO_STR, MMI_STR(dlerror()));
        dlclose(libsnscal);
        return;
    }
}
