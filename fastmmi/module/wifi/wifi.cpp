...
static int start_test(char *buf, uint32_t size) {
    ...
    int loop = 0;
    system("/system/bin/wpa_supplicant -B -iwlan0 -Dnl80211 -c/etc/mmi/wpa_supplicant_test.conf -O/data/misc/wifi/sockets -ddd");
    sleep(2);	
...
for(loop = 0; loop < 6; loop ++)
	{
	    ...
	    memset(buf, 0 ,SIZE_1K);
           memset(tmp, 0 ,256);
	    sleep(5);
	    char *args[] = {(char *)"/system/bin/wpa_cli", (char *)"-i", (char *)"wlan0", (char *)"-p", (char *)"/data/misc/wifi/sockets", (char *)"stat", NULL};
	    execmd.cmd = "/system/bin/wpa_cli";
	    execmd.params = args;
	    execmd.pid = -1;
