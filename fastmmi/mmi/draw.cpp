...
void draw_listviews(layout * lay, SkCanvas * skcan, draw_control_t * pdraw) {
    ...
    point_t point;
    SkBitmap *bitmap = NULL, *bitmap_fail = NULL, *bitmap_pass = NULL, *bitmap_fail_item = NULL;
    ...
    bitmap_fail_item = load_image("fail_item.png", true);
    ...
    for(iter = container->get_items()->begin(); iter != container->get_items()->end(); iter++) {
        ...
        /**Draw bitmap*/
        if((obj->mod->result == FAILED || obj->mod->result == SUCCESS) && !res_tab) {
        ...
        if(res_tab){
       /*    pdraw->paint.setColor(0xFFFF0000);//result listview background color
           srect.set(obj->rect.x, obj->rect.y+10, obj->rect.x + rect->w, obj->rect.y +obj->rect.h-10);
	   skcan->drawRect(srect, pdraw->paint);*/
            pdraw->paint.setColor(SK_ColorRED);
            align_center(rect, rect->w - 10, bitmap_h, &point);
            skcan->drawBitmap(*bitmap_fail_item, point.x, point.y - bitmap_h, &pdraw->paint);
        } else {
            pdraw->paint.setColor(SK_ColorWHITE);
        }
