...
static button *btn_active = NULL;
static pthread_mutex_t btn_mutex;
...
static void process_touch_up() {
   ...
   if(!btn_disable){
        ...
        for(iter = curlay->button_list.begin(); iter != curlay->button_list.end(); iter++) {
		    ...
		    ALOGD("%s(ellis):[%s] btn is set_disabled(true)\n", __FUNCTION__,curlay->module->module);
...
void init_input() {
    ...
    pthread_mutex_init(&btn_mutex, NULL);
...
void *input_handle_thread(void *) {
    ...
    while(1) {
        ...
        if((r != NULL) && (r->cb != NULL) && (r->module != NULL)) {
            ...
            if((time_diff < 300 * 1000) && (time_diff > 0)) {
                MMI_ALOGW("Input interval time=%llu, input invalid.\n", time_diff);
                if(btn_active != NULL && btn_active->get_disabled()){
                    module_info *t = (module_info *)(r->module);
                    color_t *btn_color = btn_active->get_color();
                    ALOGD("%s(input diff time invalid):[%s] btn is set_disabled(false)\n", __FUNCTION__,t->module);
	            btn_active->set_disabled(false);
	            btn_active = NULL;
	        }
                continue;
            } else
