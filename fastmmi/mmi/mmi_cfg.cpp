...
static int create_mmi_cfg(const char *cfg_path, list < cfg_module_info * >*ordered_module) {
    ...
    for(iter = ordered_module->begin(); iter != ordered_module->end(); iter++) {
        ...
        if(!strcmp(module->module, "SECOND_LCD")){
	    if(!strcmp(board_info.soc_name,"MSM8937")){
                ALOGE("soc_name is %s, module[%s],set second lcd enabled", board_info.soc_name,module->module);
                module->is_disable = false;
            }else{
                ALOGE("set second lcd disabled");
                module->is_disable = true;
            }
        }
