...
int load_config(const char *path, unordered_map < string, module_info * >*modules, list < module_info * >*clients, unordered_map < string, string >*label_modules, int _sim_num, int _lcd) {
    char module[1024] = { 0, };
    char line[1024] = { 0, };
    char indicator = '=';
    ...
    while(fgets(line, sizeof(line), file) != NULL) {
        ...
        if(module[0] != '\0') {
            ...
            if(*pname != '\0' && *pvalue != '\0') {
                ...
                MMI_ALOGI("%s=%s \n", pname, pvalue);
                if(!strcmp(pname,"display_name")){
                    MMI_ALOGI("Reverse value->module %s=%s \n", get_string((string) pvalue), ((string) module).c_str());
                    (*label_modules)[(string)module] = (string) module;
                }

				if(!strcmp(module,"SECOND LCD")){
					if(_lcd == 1){
	                	if(cur_mod != NULL){
							if(!strcmp(pname,KEY_ENABLE)){
								MMI_ALOGI("Parse CFG:FCT only one LCD.Disable the SECOND_LCD\n");
								cur_mod->config_list[(string) pname] = "0";
							}
	                	}	
					}
				}
				...
				if(_sim_num == 0){
					if(!strcmp(module,"SIMCARD1")||!strcmp(module,"SIMCARD2")||!strcmp(module,"GPS")){
	                	if(cur_mod != NULL){
							if(!strcmp(pname,KEY_ENABLE)){
								cur_mod->config_list[(string) pname] = "0";
							}
					...
					MMI_ALOGI("Parse CFG:FCT TEST with Two Simcards\n");
				}
            }
        }
    }
