                            ...
                            MUX_handle_type *output_handle)
{
  ...
  if(Params && (file_format == MUX_FMT_MP2))
  {
    ...
    */
    ...
    int nVal = 0;
    int nRet;
    nRet = PargeCfgForIntValueForKey((char *)"/system/etc/wfdconfig.xml",
                                     (char *)"DisableFillerNalU", &nVal);
                            ...
                            bool bHeader)
{
    ...
    if(bHeader == true)
    {
       ...
       if ((ulTempBufferSize < MAX_UINT32) && (sps_size > 0) &&
           (ulTempBufferSize >=sps_size + 8))
       {
         pps_size = *nSize - sps_size - 4 - 4;
       }
       if ((ulTempBufferSize < MAX_UINT32) &&
           (ulTempBufferSize >= (sps_size + 4)) && sps_size > 0)
       {
         memmove((uint8 *)pData + 3,
                 (uint8 *)pData + 4, sps_size);
       }
       if ((ulTempBufferSize < MAX_UINT32) && (pps_size > 0) &&
           (ulTempBufferSize >= (pps_size + sps_size + 8)))
       {
         memmove((uint8 *)pData + 3/*marker in videofmt format*/ + sps_size
...
MP2BaseFile::~MP2BaseFile()
{
  ...
  if(m_video_frame_timer)
  {
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "MP2BaseFile::releasing MM timer");
...
void MP2BaseFile::handle_close ()
{
  ...
  if(m_video_frame_timer)
  {
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "MP2BaseFile::stopping MM timer in handle_close");
    MM_Timer_Stop(m_video_frame_timer);
   // m_video_frame_timer = NULL;
  }
}
                                             ...
                                             MUX_sample_info_type  *sample_info)
{
  ...
  if(m_MP2PCRTSPacket)
  {
    ...
    uint8 nAdaptationControl =  MUX_MP2_ADAPTATION_ONLY;
    ...
    if(index < TS_PKT_SIZE)
    {
      ...
      if(index < TS_PKT_SIZE )
      {
        ...
        m_MP2PCRTSPacket[4] = m_MP2PCRTSPacket[4] + bytes_to_stuff;
                                       ...
                                       uint8  *PES_Header )
{
  ...
  if(PES_Header)
  {
    ...
    {
      m_PTS_DTS_Flags = 2;
      /* Computing PES_header_data_len for all audio codecs as per spec*/
      switch(m_Params->streams[stream_number].subinfo.audio.format)
      {
        case MUX_STREAM_AUDIO_PCM:
        case MUX_STREAM_AUDIO_MPEG4_AAC:
    ...
    if(m_Params->streams[stream_number].type == MUX_STREAM_AUDIO)
    {
      switch(m_Params->streams[stream_number].subinfo.audio.format)
      {
        ...
        case MUX_STREAM_AUDIO_AC3:
          ...
          PES_Header[index] = 0xFF;
          index += 1;
          /* MP2BaseFile:: Mux Client will add the 4 byte LPCM private header
             as part of PES payload following this */
          break;
        default:
          break;
      }
