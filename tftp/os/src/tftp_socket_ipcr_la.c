...
#include <string.h>
  ...
  #define ipcr_la_send      sim_ipcr_socket_la_send
  #define ipcr_la_sendto    sim_ipcr_socket_la_sendto
  #define ipcr_la_recv      sim_ipcr_socket_la_recv
  ...
  #define ipcr_la_send      send
  #define ipcr_la_sendto    sendto
  #define ipcr_la_recv      recv
                  ...
                  int32 *sent_size)
{
  ...
  result = ipcr_la_send (*socket_handle, buffer, buffer_size, 0);
  if (result == -1)
  {
    sock_res = TFTP_SOCKET_RESULT_FAILED;
  }
                  ...
                  uint32 timeout_in_ms, int32 *recd_size)
{
  ...
  result = ipcr_la_recv(*socket_handle, buffer, buffer_size, 0);
  if (result < 0)
  {
    sock_res = TFTP_SOCKET_RESULT_FAILED;
  }
