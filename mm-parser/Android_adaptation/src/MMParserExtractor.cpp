...
char FileName[] =  "/data/misc/media/bitstream";
    ...
    if(albBuffer != NULL) {
        ...
        if(albStatus==FILE_SOURCE_SUCCESS && nMetaBuff > 0 ) {
            ...
            //! If metadata is in UTF16 string format, set it as char16 string
                ...
                String8 temp;
                temp.setTo((const char16_t*)albBuffer, nMetaBuff/2);
...
uint32 ReadBits(sBitBuffer* pBitBuf, uint32 ulNumBitstoRead)
{
  ...
  {
    LOGE("ReadBits, requested more than 32 bits %u",ulNumBitstoRead);
    return 0;
  }
  else if (pBitBuf->pBsCur > pBitBuf->pBsEnd)
  {
    LOGE("ReadBits, Buffer exceeds the upper limit, start %p, end %p",
...
bool MMParserExtractor::checkDataPartitionFromCodecData(uint8_t *data,
                                                        size_t size) {
    bool retVal = false;
    ...
    if (!data || (size < kMinCsdSize)) {
        LOGV("Invalid Codec Data (expected at least %zu bytes)", kMinCsdSize);
        return retVal;
    }
    ...
    if (isStartCode) {
        retVal = checkDataPartitionFlagFromVOL((uint8_t*) &data[startCodeOffset],
...
bool MMParserExtractor::checkDataPartitionFlagFromVOL(uint8_t *data,
                                                      size_t size) {
    bool retVal = false;
    ...
    LOGV("Checking for MPEG4 DP bit");
    sBitBuffer sBitBuf;
    memset(&sBitBuf, 0, sizeof(sBitBuffer));
    sBitBuf.pBsCur = data + 4;
    sBitBuf.pBsEnd = data + size;
    ReadBits(&sBitBuf, 1); // random accessible vol

    unsigned videoObjectTypeIndication = ReadBits(&sBitBuf, 8);
    if (videoObjectTypeIndication == 0x12u) {
        LOGW("checkDPFromVOLHeader: videoObjectTypeIndication:%u",
               videoObjectTypeIndication);
        return false;
    }

    unsigned videoObjectLayerVerid = 1;
    if (ReadBits(&sBitBuf, 1)) {
        videoObjectLayerVerid = ReadBits(&sBitBuf, 4);
        ReadBits(&sBitBuf, 3); // video_object_layer_priority
        LOGV("checkDPFromVOLHeader: videoObjectLayerVerid:%u",
              videoObjectLayerVerid);
    }

    if (ReadBits(&sBitBuf, 4) == 0x0f) { // aspect_ratio_info
        LOGV("checkDPFromVOLHeader: extended PAR");
        ReadBits(&sBitBuf, 8); // par_width
        ReadBits(&sBitBuf, 8); // par_height
    }

    if (ReadBits(&sBitBuf, 1)) { // vol_control_parameters
        ReadBits(&sBitBuf, 2);  // chroma_format
        ReadBits(&sBitBuf, 1);  // low delay
        if (ReadBits(&sBitBuf, 1)) { // vbv parameters
            ReadBits(&sBitBuf, 15); // first_half_bit_rate
            ReadBits(&sBitBuf, 1);  // marker bit
            ReadBits(&sBitBuf, 15); // latter_half_bit_rate
            ReadBits(&sBitBuf, 1);  // marker bit
            ReadBits(&sBitBuf, 15); // first_half_vbv_buffer_size
            ReadBits(&sBitBuf, 1);  // marker bit
            ReadBits(&sBitBuf, 3);  // latter_half_vbv_buffer_size
            ReadBits(&sBitBuf, 11); // first_half_vbv_occupancy
            ReadBits(&sBitBuf, 1);  // marker bit
            ReadBits(&sBitBuf, 15); // latter_half_vbv_occupancy
            ReadBits(&sBitBuf, 1);  // marker bit
        }
    }

    unsigned videoObjectLayerShape = ReadBits(&sBitBuf, 2);
    if (videoObjectLayerShape != 0x00u /* rectangular */) {
        LOGV("checkDPFromVOLHeader: videoObjectLayerShape:%x",
               videoObjectLayerShape);
        return false;
    }

    ReadBits(&sBitBuf, 1);  // marker bit
    unsigned vopTimeIncrementResolution = ReadBits(&sBitBuf, 16);
    ReadBits(&sBitBuf, 1); // marker bit
    if (ReadBits(&sBitBuf, 1)) {  // fixed_vop_rate
        ...
        ReadBits(&sBitBuf, numBits);  // fixed_vop_time_increment
    }

    ReadBits(&sBitBuf, 1);  // marker bit
    ReadBits(&sBitBuf, 13); // video_object_layer_width
    ReadBits(&sBitBuf, 1);  // marker bit
    ReadBits(&sBitBuf, 13); // video_object_layer_height
    ReadBits(&sBitBuf, 1);  // marker bit
    ReadBits(&sBitBuf, 1);  // interlaced
    ReadBits(&sBitBuf, 1);  // obmc_disable
    unsigned spriteEnable = 0;
    if (videoObjectLayerVerid == 1) {
        spriteEnable = ReadBits(&sBitBuf, 1);
    } else {
        spriteEnable = ReadBits(&sBitBuf, 2);
    }

    if (spriteEnable == 0x1) { // static
        int spriteWidth = ReadBits(&sBitBuf, 13);
        LOGV("checkDPFromVOLHeader: spriteWidth:%d", spriteWidth);
        ReadBits(&sBitBuf, 1);  // marker bit
        ReadBits(&sBitBuf, 13); // sprite_height
        ReadBits(&sBitBuf, 1);  // marker bit
        ReadBits(&sBitBuf, 13); // sprite_left_coordinate
        ReadBits(&sBitBuf, 1);  // marker bit
        ReadBits(&sBitBuf, 13); // sprite_top_coordinate
        ReadBits(&sBitBuf, 1);  // marker bit
        ReadBits(&sBitBuf, 6);  // no_of_sprite_warping_points
        ReadBits(&sBitBuf, 2);  // sprite_warping_accuracy
        ReadBits(&sBitBuf, 1);  // sprite_brightness_change
        ReadBits(&sBitBuf, 1);  // low_latency_sprite_enable
    ...
    if (ReadBits(&sBitBuf, 1)) { // not_8_bit
        ReadBits(&sBitBuf, 4);  // quant_precision
        ReadBits(&sBitBuf, 4);  // bits_per_pixel
    }
    ...
    if (ReadBits(&sBitBuf, 1)) { // quant_type
        if (ReadBits(&sBitBuf, 1)) { // load_intra_quant_mat
            unsigned IntraQuantMat = 1;
            for (int i = 0; i < 64 && IntraQuantMat; i++) {
                 IntraQuantMat = ReadBits(&sBitBuf, 8);
            }
        }

        if (ReadBits(&sBitBuf, 1)) { // load_non_intra_quant_matrix
            unsigned NonIntraQuantMat = 1;
            for (int i = 0; i < 64 && NonIntraQuantMat; i++) {
                 NonIntraQuantMat = ReadBits(&sBitBuf, 8);
            }
        }
    } /* quantType */

    if (videoObjectLayerVerid != 1) {
        unsigned quarterSample = ReadBits(&sBitBuf, 1);
        LOGV("checkDPFromVOLHeader: quarterSample:%u",
                quarterSample);
    }

    ReadBits(&sBitBuf, 1); // complexity_estimation_disable
    ReadBits(&sBitBuf, 1); // resync_marker_disable
    unsigned dataPartitioned = ReadBits(&sBitBuf, 1);
    if (dataPartitioned) {
        retVal = true;
    }

    LOGV("checkDPFromVOLHeader: DP:%u", dataPartitioned);
    return retVal;
}
...
status_t MMParserExtractor::readMetaData() {
    ...
    property_get("audio.parser.ip.buffer.size", audioParserBuf, "0");
            ...
            if (majorType == FILE_SOURCE_MJ_TYPE_AUDIO){
                ...
                if( (minorType == FILE_SOURCE_MN_TYPE_AC3) ||
                    ...
                    if ((true == gbLocalPlayback) && mUseBigBuffers &&
                        ...
                        track->meta->setInt32(kKeyMaxInputSize, audParserInpBufSize);
                        track->meta->setInt32(kKeyIsByteMode, 1);
                    } else if(FILE_SOURCE_AVI != fileFormat) {
                      //! AVI does not need Frame by Frame mode for MP3 Codec
                        configStatus = m_pFileSourceWrapper->SetConfiguration(trackInfo.id, &pitem, FILE_SOURCE_MEDIA_OUTPUT_SINGLE_AUDIO_FRAME);
                    }
...
//! If any customer needs downgrade, then on need basis decision to be taken
                        ...
                        while(buffer!=NULL && ulIndex < 4) {
                            ...
                            if(mstatus==FILE_SOURCE_DATA_OK) {
                                if(1 == ulIndex) {
                                    ...
                                    LOGV(" kKeyVorbisData set, size %lu \n", ulBufSize);
                                } else if (3 == ulIndex) {
                ...
                else if(FILE_SOURCE_MN_TYPE_OPUS == minorType)
                {
                  ...
                  LOGV("OPUS nCodecDelay %lu,nSeekPrerollDelay %llu",sOpus_params.ullCodecDelay,sOpus_params.ullSeekPreRoll);
                    ...
                    //Get the size of the format block.
                    status = m_pFileSourceWrapper->GetFormatBlock(trackInfo.id, NULL, &nFormatBlockSize);
                    LOGV(" ALAC getFormatBlock size = %lu \n", nFormatBlockSize);
                    if((status==FILE_SOURCE_SUCCESS) && nFormatBlockSize!=0) {
                        uint8_t *buffer = new uint8_t[nFormatBlockSize];
                        if(buffer!=NULL) {
                            status = m_pFileSourceWrapper->GetFormatBlock(trackInfo.id, buffer, &nFormatBlockSize);
                    ...
                    if(buffer!=NULL) {
                        ...
                        if(status==FILE_SOURCE_SUCCESS) {
                            ...
                            if (false == bRawCodecData) {
                                ...
                                if(FILE_SOURCE_MN_TYPE_MPEG4 == minorType) {
                                    if (checkDataPartitionFromCodecData((uint8_t*) buffer, nFormatBlockSize)) {
                                        track->meta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_VIDEO_MPEG4_DP);
                                    }
                                }
                                ...
                                LOGV("getFormatBlock size VP9 = %lu \n", nFormatBlockSize);
                            ...
                            if (mSetBuffers) {
                                ...
                                LOGV("Total size of csd is %d", mFormatBlock->size());
                            }
                        } else {
                            LOGE(" getFormatBlock failed \n");
                        }
                ...
                if (m_pFileSourceWrapper->IsDrmProtection()) {
                    ...
                    if (!m_pFileSourceWrapper->IsSeekDenied()) {
                        ...
                        for (uint64 lastSyncSampleTime = 0, syncSample = 0;
                            ...
                            //! Find sync sample at 1/4th of duration
                            if(fileFormat == FILE_SOURCE_ASF){
                                 lastSyncSampleTime = DurationinMS/4;
                            }
                            uint32 bytesRead = 0;
...
status_t MMParserSource::start(MetaData *params) {
    ...
    if(FILE_SOURCE_MN_TYPE_G711_ALAW == minorType ||
      ...
      if (mFormat->findInt32(kKeyMaxInputSize, &ulSize) && ulSize) {
        LOGV("MMParserSource updated max size to %lu", ulSize);
        max_size = ulSize;
      }
    }
    ...
    if(((true == gbLocalPlayback) &&
        ...
        if(mMMParserExtractor->mUseBigBuffers &&
            ...
            LOGD("Overiding Parser out buffer size  = %d \n", max_size);
        } else {
            LOGD("Using default parser out buffer size  = %d \n", max_size);
        }
    }
...
status_t MMParserSource::read(
    ...
    *out = NULL;
    ...
    if (options && options->getSeekTo(&seekTimeUs, &seekMode)) {
        ...
        LOGV(" MMParserSource::read Seek seekTimeUs =%lld,seekMode %d \n",seekTimeUs,seekMode);
        //! Seektime needs to be in milli-sec units, Convert from Us to Ms.
        seekTimems = (seekTimeUs / 1000);
        if (ReadOptions::SEEK_PREVIOUS_SYNC == seekMode)
          m_eSeekMode = FS_SEEK_PREV;
        LOGV(" MMParserSource::read Seek seekTimems =%lu , mTrackId = %d\n", seekTimems,mTrackId);
                   ...
                   (m_pFileSourceWrapper->mVideoMinorType == FILE_SOURCE_MN_TYPE_H264))
        {
          ...
          while((ulIdx < size) &&
            ...
            if ((nalStart[0] & 0x1f) == 0x06 && nSeiCount < 6) {
              ...
              max_sei[nSeiCount-1].nalSize = nalSize;
              max_sei[nSeiCount-1].nalOffset = (size_t)(nalStart - data);
              tempPtr = (data + ulIdx);
              if(ulIdx < size)
                dataSize = size - ulIdx;
            }
...
status_t MMParserSource::setBuffers(const Vector<MediaBuffer *> &pBuffers) {
   LOGI("Using codec-supplied buffers, size is %d", pBuffers.size());
   CHECK(mSetBuffers);
