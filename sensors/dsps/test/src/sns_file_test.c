...
test_req_open( test_control_s *test_control, char *file_name, char *mode )
{
  ...
  strlcpy( sensor1_req->path_name, file_name, sizeof(sensor1_req->path_name) );
  sensor1_req->mode_len = strlen( mode );
  strlcpy( sensor1_req->mode, mode, sizeof(sensor1_req->mode) );
...
test_req_write( test_control_s *test_control, int32_t fildesc, char const *buf )
{
  ...
  strlcpy( (char*)sensor1_req->buf, buf, sizeof(sensor1_req->buf) );
