/*============================================================================
  ...
  @brief
  FaceNShake class implementation.
...
/*============================================================================
  FaceNShake Constructor
============================================================================*/
FaceNShake::FaceNShake(int handle)
    :SAMSensor(handle)
{
    if(getAttribOK() == true) {
        svc_num = SNS_SAM_FNS_SVC_ID_V01;
        trigger_mode = SENSOR_MODE_EVENT;
        HAL_LOG_INFO("%s: handle:%d", __FUNCTION__, handle);
        setName("Face-N-Shake");
        setVendor("QTI");
        setType(SENSOR_TYPE_GESTURE_FACE_N_SHAKE);
...
/*============================================================================
  FaceNShake Destructor
============================================================================*/
FaceNShake::~FaceNShake()
{
...
int FaceNShake::enable(int en)
{
    ...
    if (enabled == en) {
        HAL_LOG_INFO("FNS is already enabled/disabled %d", enabled);
        return 0;
    }
...
===========================================================================*/
void FaceNShake::processResp(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    if(true != error ) {
        ...
        case SNS_SAM_FNS_ENABLE_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_FNS_ENABLE_RESP_V01", __FUNCTION__);
            instance_id = ((sns_sam_fns_enable_resp_msg_v01 *)msg_ptr)->instance_id;
            break;
        case SNS_SAM_FNS_CANCEL_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_FNS_CANCEL_RESP_V01", __FUNCTION__);
            /* Reset instance ID */
            instance_id = 0xFF;
            break;
        case SNS_SAM_FNS_GET_ATTRIBUTES_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_FNS_GET_ATTRIBUTES_RESP_V01", __FUNCTION__);
...
/*===========================================================================
  FUNCTION:  processInd
===========================================================================*/
void FaceNShake::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    switch(msg_hdr->msg_id) {
        case SNS_SAM_FNS_REPORT_IND_V01:
            HAL_LOG_DEBUG("%s: SNS_SAM_FNS_REPORT_IND_V01", __FUNCTION__);
        ...
        case SNS_SAM_FNS_ERROR_IND_V01:
            HAL_LOG_ERROR("%s: SNS_SAM_FNS_ERROR_IND_V01", __FUNCTION__);
            error = true;
            break;
        default:
    ...
    if ( error == false ) {
        ...
        HAL_LOG_VERBOSE("%s: FNS data[0]: %f SAM TS: %u HAL TS:%lld elapsedRealtimeNano:%lld",
