...
int SMGRSensor::enable(int en)
{
    ...
    char raw_mode[PROPERTY_VALUE_MAX] = "false";
    property_get( HAL_PROP_RAWDATAMODE, raw_mode, "false" );
    if ((!strncmp("true", raw_mode, 4)) && en) {
...
void SMGRSensor::processBufferingInd(Sensor** mSensors, sns_smgr_buffering_ind_msg_v01* smgr_ind)
{
    ...
    * in the buffering request */
    ...
    if ((0 < smgr_ind->Indices_len ) &&
        ...
        HAL_LOG_DEBUG("%s: Handle=%d mSensors[handle]=%d mSensors[handle]->isBatchingEnable()=%d",
