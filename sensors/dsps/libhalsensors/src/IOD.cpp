...
int IOD::enable(int en)
{
    ...
    if (en) {
        ...
        HAL_LOG_DEBUG("%s: ts:%ld sunrise_ts:%ld sunset_ts:%ld",
...
void IOD::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    switch(msg_hdr->msg_id) {
        case SNS_SAM_IOD_REPORT_IND_V01:
            HAL_LOG_VERBOSE("%s: Output Class: %d Unknown Reason: 0x0%x Confidence: %d ts: %u",
                          __FUNCTION__, sam_iod_rpt_ptr->report_data.output_class,
                          sam_iod_rpt_ptr->report_data.reason_unknown,
    ...
    if (error == false) {
        ...
        HAL_LOG_DEBUG("%s: IOD: Output Class: %d SAM TS: %u HAL TS:%lld elapsedRealtimeNano:%lld",
