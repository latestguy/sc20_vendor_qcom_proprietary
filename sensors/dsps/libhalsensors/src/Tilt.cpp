...
void Tilt::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    if (error == false) {
        ...
        HAL_LOG_VERBOSE("%s: TILT: %f, %f, %f, SAM TS: %u HAL TS:%lld elapsedRealtimeNano:%lld", __FUNCTION__,
                         sensor_data.data[0], sensor_data.data[1], sensor_data.data[0], timestamp,
