...
void  StepCounter::manageBatch( sigval_t val )
{
    ...
    if ( sc_handle->enabled ) {
        ...
        if( batch_period > 0 ) {
            ...
            timer_settime( sc_handle->sc_timer, TIMER_ABSTIME, &timer, NULL );
        }
...
/*============================================================================
  initTimer
============================================================================*/
int StepCounter::initTimer(void)
{
...
int StepCounter::enable(int en)
{
    ...
    if (en) {
        ...
        if( batching ) {
            ...
            if ( initTimer() ) {
                sendCancel();
                HAL_LOG_ERROR("%s: Timer init fail", __FUNCTION__);
                enabled = 0;
                return -1;
            }
        }
        ...
        sendCancel();
        if( step_counter_is_timer_created ) {
            if (timer_delete (sc_timer) < 0) {
                HAL_LOG_ERROR(" %s:timer_delete fail", __FUNCTION__);
            } else {
                step_counter_is_timer_created = false;
            }
        }
    ...
    for (i = 0; i < count; i++) {
        ...
        */
        if ((step_counter_running_total == 0) ||
            ...
            HAL_LOG_VERBOSE("%s: STEP COUNTER: steps %" PRIu64", running_total %" PRIu64" SAM TS: %u HAL TS:%lld elapsedRealtimeNano:%lld",
                            __FUNCTION__, steps, step_counter_running_total, curr_sample->timestamp, la_sample.timestamp,
