...
/**
 * WORKAROUND: qmi_client_release_async() blocks for 200ms so handle
 * QMI tear-down with a dedicated worker thread
 */
static void * qmi_client_release_worker_thread(void *qmi_client_hndl)
{
  uint32_t rc;
  qmi_client_type *qmi_client_p;

  qmi_client_p = (qmi_client_type *)qmi_client_hndl;
  rc = qmi_client_release(*qmi_client_p);
  if (rc)
    SNS_PRINTF_STRING_ERROR_1( SNS_DBG_MOD_APPS_MAIN,
                               "ERROR sensor1_close failed %i", rc );
  free(qmi_client_p);
...
sns_acm_mr_close( uint32_t ext_clnt_id )
{
  ...
  for( i = 0; i < SNS_ACM_MAX_SVC_ID; i++ )
  {
    ...
    {
      SNS_PRINTF_STRING_LOW_2( SNS_DBG_MOD_ACM, "Releasing QMI client %i %i",
                               ext_clnt_id, i );
      /*
       * qmi_client_release_async() blocks for 200ms so create a
       * worker thread to handle the work. ext_clnt_id is free to be recycled,
       * so we must make a copy of our QMI handle to pass to the worker thread
       */
      qmi_client_p = malloc(sizeof(*qmi_client_p));
      if (qmi_client_p == NULL) {
        SNS_PRINTF_STRING_ERROR_1( SNS_DBG_MOD_ACM, "Error releasing QMI client %i, malloc failed",
                                   ext_clnt_id );
        return;
      }
      memcpy(qmi_client_p, &(external_clients[ ext_clnt_id ].client_handles[ i ]),
             sizeof(*qmi_client_p));
      err = pthread_create( &worker_thread, NULL, qmi_client_release_worker_thread,
                            (void *)qmi_client_p);
      err |= pthread_detach( worker_thread );
      if ( err ) {
        SNS_PRINTF_STRING_ERROR_1( SNS_DBG_MOD_ACM, "Error releasing QMI client %i", ext_clnt_id );
      }
      else {
        SNS_PRINTF_STRING_LOW_1( SNS_DBG_MOD_ACM, "Client release deferred %i", ext_clnt_id );
      }
