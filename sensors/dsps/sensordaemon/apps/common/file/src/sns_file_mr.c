    ...
    void **connection_handle )
{
  ...
  if( SNS_SUCCESS != sns_file_csi_connect( connection_handle ) )
  {
    SNS_PRINTF_STRING_HIGH_0( SNS_DBG_MOD_APPS_FILE,
                              "sns file service connect fail" );
  }
  return 0;
}
...
sns_file_mr_csi_disconnect( void *connection_handle, void *service_cookie )
{
  ...
  if( SNS_SUCCESS != sns_file_csi_disconnect( connection_handle ) )
  {
    SNS_PRINTF_STRING_HIGH_0( SNS_DBG_MOD_APPS_FILE,
                              "sns file service disconnect fail" );
  }
}
    ...
    void *service_cookie )
{
  ...
  smr_err = sns_file_handle( &smr_header, connection_handle, req_c_struct, &smr_resp_msg );
