 ...
 */
STATIC sns_sam_err
sns_sam_smgr_handle_enable_resp( sns_sam_resp *respMsg )
{
  ...
  else
  {
    ...
    if( respMsg->sensorReq->deferredBatchReq )
    {
      sns_sam_send_batch_req( respMsg->sensorReq );
      respMsg->sensorReq->deferredBatchReq = false;
 ...
 */
STATIC void
sns_sam_handle_smgr_resp( sns_sam_resp *respMsg )
{
 ...
 */
STATIC sns_sam_err
sns_sam_smgri_handle_query_resp( sns_sam_resp *respMsg )
{
  sns_sam_err err = SAM_ENONE;
  uint32_t i;
  sns_q_link_s *qCurr;
 ...
 */
STATIC void
sns_sam_handle_smgri_resp( sns_sam_resp *respMsg )
{
  sns_sam_err err = SAM_EFAILED;
