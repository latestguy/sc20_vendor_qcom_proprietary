 ...
 * a new sensor request for the given sensor.
 *
    ...
    sns_sam_enable_req *enableReqMsg )
{
  ...
  sns_sam_sensor_req *sensorReq, *reqToDelete = NULL;
  ...
  {
    reqToDelete = algoInst->sensorRequests[ availableIndex ];
    algoInst->sensorRequests[ availableIndex ] = NULL;
    ...
    if( NULL != sensorReq )
    {
      ...
      // Check whether we found a shared copy of the same sensor request
      if( NULL != reqToDelete && reqToDelete != sensorReq )
      {
        sns_sam_remove_sensor_req( reqToDelete );
      }

      //Found existing sensorReq, free up the enableReqMsg
      SNS_SAM_MEM_FREE( (void*)enableReqMsg->msg.buf );
    ...
    else
    {
      ...
      // Remove existing sensor request before updating any other requests
      if( NULL != reqToDelete )
      {
        sns_sam_remove_sensor_req( reqToDelete );
      }
  ...
  sns_sam_sensor_uid const *sensorUID )
{
  ...
  if( SAM_ESTATE == err )
  {
    int index = sns_sam_find_free_sensor_req_index( sensorUID, algoInst );
    if( -1 != index && NULL != algoInst->sensorRequests[ index ] )
    {
      sns_sam_sensor_req *sensorReq = algoInst->sensorRequests[ index ];
      algoInst->sensorRequests[ index ] = NULL;
  ...
  sns_sam_sensor_uid const *sensorUID, uint32_t batchPeriod )
{
  ...
  if( sns_sam_is_smgr_sensor( sensor ) )
  {
  ...
  if ( UINT16_MAX == sensorRequest->instanceID )
  {
    sensorRequest->deferredBatchReq = true;
...
sns_sam_inform_report_change( sns_sam_algo_inst const *algoInst )
{
  ...
  q16_t minReportPeriod = INT32_MAX;
  sns_q_link_s *qCurr = NULL;
  ...
  SNS_PRINTF_STRING_LOW_1( samModule, "Client report change for algo:%x ",
                           algoInst->algorithm->sensor.sensorUID);
        ...
        qCurr = sns_q_next( &algoInst->clientRequests, qCurr ) )
  {
    ...
    if( clientReq->clientAttr.reportMode == SNS_SAM_REPORT_MODE_PERIODIC )
    {
    ...
    {
      // async, sync, one shot
      if( clientReq->batchPeriod != 0 )
      {
        if( clientReq->batchPeriod < minReportPeriod )
        {
          minReportPeriod = clientReq->batchPeriod;
        }
      }
      else
      {
  ...
  SNS_PRINTF_STRING_LOW_1( samModule,
    "Inform algorithm of min report period:%i", minReportPeriod);
...
bool
sns_sam_is_smgr_sensor( sns_sam_sensor const *sensor )
{
  int32_t errQMI = 0;
...
sns_sam_send_batch_req( sns_sam_sensor_req *sensorRequest )
{
  ...
  SNS_OS_FREE((void *)reqMsg.msg.buf);
