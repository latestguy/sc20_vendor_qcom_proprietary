...
sns_sam_copy_output( sam_client_req *clientReq, sns_sam_algo_output const *outputData )
{
  ...
  else
  {
    ...
    outputCopy = SNS_OS_SENSOR_HEAP_MALLOC( SNS_SAM_DBG_MOD, sizeof(sns_sam_algo_output) );
  }
...
/**
 * Whether the specified client request object is still valid.  Will compare
 * against all current client requests.
 *
 ...
 */
SNS_SAM_UIMAGE_CODE STATIC bool
sns_sam_valid_client_request( sam_client_req const *clientReq )
{
       ...
       qCurrAI = sns_q_next( &algoInstQ, qCurrAI ) )
  {
    ...
    {
      if( clientReq == (sam_client_req*)qCurrCR )
      {
        return true;
      }
    }
...
{
  if( SNS_PROC_SSC_V01 == clientReq->clientAttr.notifySuspend.proc_type ||
...
SNS_SAM_UIMAGE_CODE void
sns_sam_handle_batch_send_ind( sam_client_req *clientReq )
{
  sns_sam_err errFW;
  sns_sam_err errAlgo;
  sns_sam_algo_inst *algoInst = clientReq->algoInstance;
...
sns_sam_algo_run( sns_sam_ind *indMsg, sns_sam_algo_inst *algoInst )
{
  ...
  errAlgo = algoInst->algorithm->algoMsgAPI->sns_sam_algo_process_ind(
      &algoInst->cbFuncs, &indMsg->msg, (sns_sam_input_q *) &algoInst->algoInputQ );

  sns_profiling_log_qdss( SNS_SAM_PROCESS_IND_EXIT, 2,
    (uint32_t)algoInst->algorithm->sensor.sensorUID, (uint32_t)algoInst );

  /* TODO - revisit the errAlgo check to address scenarios where buffering indication
     is processed partially and valid input added to queue before a failure is observed.
     In this case, there is input for the algorithm to consume. */
  if( SAM_ENONE == errAlgo && 0 < algoInst->algoInputQ.inputData.cnt )
  {
    ...
    if( algoInst->algoInputQ.inputData.cnt > SAM_ALGO_MAX_INPUT_Q_SIZE )
    {
      ...
      SNS_PRINTF_STRING_ERROR_3( samModule,"Flushing input queue for algo %x, queue cnt %i exceeds max limit:%i",
                                 algoInst->algorithm->sensor.sensorUID,
                                 algoInst->algoInputQ.inputData.cnt,
                                 SAM_ALGO_MAX_INPUT_Q_SIZE);

      for( j = 0; j < queueLength &&
           NULL != ( inputData = sns_q_get( (sns_q_s *)&algoInst->algoInputQ ) ); j++ )
      {
          sns_sam_free_io( algoInst, (intptr_t) inputData );
      }
...
sns_sam_handle_ind( sns_sam_ind *indMsg )
{
  ...
  /* check if there's low memory signal needed to handle first */
...
sns_sam_handle_batch_timer( void )
{
  ...
  do
  {
    ...
    if( NULL != clientEvent )
    {
      if( sns_sam_valid_client_request( clientEvent->clientReq ) )
      {
