...
static bool sdk_white_list[MAX_SDK_WHITELIST] = {false, false, false, false, true, true, false, false,   //AMD=4,RMD=5
                                                    ...
                                                    true, false, false, false, false, false, true, true, //DPC=48, IOD=54, AUTOCAL=55
                                                    false, false, false, false, false, false, false, false};
    ...
    if( NULL != ctl_p ) {
      ...
      if( SENSOR1_MSG_TYPE_RESP == msg_type ) {
        ...
        if( NULL != pkt_p ) {
          ...
          if( -1 == err ) {
            SNS_PRINTF_STRING_ERROR_3( SNS_DBG_MOD_APPS_MAIN,
                                       "Error writing RESP to ctl sock fd %d, "
                                       "err %d, service %d",
                                       ctl_p->sock_fd, errno, pkt_p->svc_num );
            ...
            if( -1 == err ) {
              SNS_PRINTF_STRING_ERROR_1( SNS_DBG_MOD_APPS_MAIN,
                  "Error sensor1 client %d is not responding, closing client", ctl_p->sock_fd);
              /*
               * Teardown the unresponsive client socket
               * We hope the client can unblock itself with delivery of
               * SENSOR1_MSG_TYPE_BROKEN_PIPE
               */
               sensor1_close(ctl_p->sensor1_hndl);
               close(ctl_p->sock_fd);
               ctl_p->sock_fd = -1; /* Flag for cleanup in sns_main_sock_handle() */
            } else {
              SNS_PRINTF_STRING_ERROR_0( SNS_DBG_MOD_APPS_MAIN,
                                         "Error resolved!" );
            }
        ...
        if( NULL != pkt_p ) {
          ...
          if( -1 == err ) {
            SNS_PRINTF_STRING_ERROR_3( SNS_DBG_MOD_APPS_MAIN,
                                       "Error writing IND to ctl sock fd %d, "
                                       "err %d, service %d",
                                       ctl_p->sock_fd, errno, pkt_p->svc_num );
