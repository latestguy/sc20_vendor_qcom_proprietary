...
#define RFSA_DL_SENSOR_CLIENT_ID	0x011013ee	/* Unique SENSOR client ID for dynamic loading */

#define DEFAULT_DIR_MODE_BITS 0700 /* The default mode bits for directories.*/
#define SHARED_DIR_MODE_BITS 0770 /* The mode bits for shared directories.*/

#define DEFAULT_FILE_MODE_BITS 0600 /* The default mode bits for files.*/
#define SHARED_FILE_MODE_BITS 0660 /* The mode bits for shared files.*/

#define UIO_QCOM_VER	"1.0"
#define UIO_QCOM_DSP_NAME	"rfsa_dsp"
#define UIO_QCOM_MDM_NAME	"rfsa_mdm"
#define UIO_QCOM_SENSOR_NAME	"rfsa_sensor"
...
struct rfsa_client_info
{
	...
	char			client_name[15]; /* Client Name */
...
/** variables used to synchronize the threads */
...
struct rfsa_client_info client_info[] = {
	...
	{
		.client_id = RFSA_DL_SENSOR_CLIENT_ID,
		.client_name = UIO_QCOM_SENSOR_NAME,
		.is_optional = 1,
		.is_inited = 0,
	},
};
...
static struct rfsa_path_table sys_path_tbl[PATH_TABLE_MAX_ITEMS] =
{
	...
	{"SNSR://", "/system/rfs/msm/adsp/", 1, RFSA_DL_SENSOR_CLIENT_ID},
	{"", "", 0, 0}
};
...
static char *data_path_list[PATH_TABLE_PATH_LEN] =
{
	...
	"/persist/rfs/msm/adsp/", /* MSM-ADSP and Sensor Read write folder */
	"/persist/rfs/msm/mpss/", /* MSM-MPSS Read write folder */

	"/persist/rfs/shared/", /* Shared for all MSM+APQ+MDM */
	"",
};

static char *shared_data_path_list[PATH_TABLE_PATH_LEN] =
{
	/* All dir paths must end in a '/'  for the directories to be
	   created. */

	"/persist/hlos_rfs/shared/", /* Shared with HLOS */
	"",
};

static char *resolved_shared_path_list[PATH_TABLE_PATH_LEN] =
{
	/* All dir paths must end in a '/'  for the directories to be
	   checked correctly. */

	"/system/rfs/msm/mpss/hlos/", /* Shared with HLOS */
	"/system/rfs/msm/adsp/hlos/", /* Shared with HLOS */
	"",
};
...
static int32_t os_mkdir (const char *path, uint16_t mode)
{
	int32_t result;
...
static int32_t os_autodir (const char *path, uint16_t mode, int32_t gid)
{
	int result = 0;
	uint32_t index = 0;
	uint32_t path_len = 0;
...
static int32_t rfsa_create_data_dirs (void)
{
	...
	while (**current_path != '\0') {
		result = os_autodir (*current_path, DEFAULT_DIR_MODE_BITS, -1);
		if (result != 0)
			goto End;
	...
	current_path = shared_data_path_list;

	while (**current_path != '\0') {
		result = os_autodir (*current_path, SHARED_DIR_MODE_BITS,
					AID_RFS_SHARED);
		if (result != 0)
			goto End;
...
static void rfsa_attempt_autodir(char *fname, uint16_t mode, int32_t gid)
{
	int32_t ret;
	ret = os_autodir (fname, mode, gid);
	...
	if (ret != 0) {
		...
		(void) os_autodir (fname, mode, gid);
	}
}

static int
rfsa_check_is_path_shared (char *file)
{
	char **current_path;
	int is_path_shared = 0;
	uint32_t length;

	current_path = resolved_shared_path_list;

	while (**current_path != '\0') {
		length = strlen(*current_path);
		if (strncmp (file, *current_path, length) == 0) {
			LOGI ("Found shared path : %s", file);
			is_path_shared = 1;
			break;
		}
		*current_path++;
	}
	return is_path_shared;
}
...
int32_t rfsa_server_create(rfsa_packet_t *packet_ptr)
{
	...
	uint16_t file_mode = DEFAULT_FILE_MODE_BITS;
	uint16_t dir_mode = DEFAULT_DIR_MODE_BITS;
	int32_t gid = -1;
	...
	is_shared_file = rfsa_check_is_path_shared (fname);

	if (is_shared_file == 1) {
		file_mode = SHARED_FILE_MODE_BITS;
		dir_mode = SHARED_DIR_MODE_BITS;
		gid = AID_RFS_SHARED;
	}
	...
	if (file_create_ptr->flags & RFSA_ACCESS_FLAG_CREATE_V01) {
		...
		rfsa_attempt_autodir(fname, dir_mode, gid);
	}
	...
	fd = open(fname, open_flags, file_mode);
...
static int32_t rfsa_server_init_all_clients(void)
{
	...
	for (i = 0; i < no_clients; i++) {
		...
		if(result != RFSA_EOK) {
			LOGE("Mmap failed for client: %s\n",
				curr_client->client_name);

			if (curr_client->is_optional == 1)
				result = RFSA_EOK;
			else
				break;
		}
...
{
	int ret;
	gid_t gid_list[4] = {AID_RFS, AID_RFS_SHARED, AID_SYSTEM, AID_NET_RAW};

	/* Drop the excess capabilities. */
	drop_excess_capabilities();

	ret = setgroups(4, gid_list);
	if (ret < 0) {
		LOGE("Failed to set supplementary-groups(ret %d) errno = %d (%s)",
			  ret, errno, strerror(errno));
		exit(1); /* It is BAD if we cannot set the correct gids */
	}

	ret = setgid(AID_RFS);
	if (ret < 0) {
		LOGE("Failed to set GID to system (ret %d) errno = %d (%s)",
			  ret, errno, strerror(errno));
		exit(1); /* It is BAD if we cannot drop to rfs from root */
	}
...
static void rfsa_wakeunlock(void)
{
	...
	write_res = write(wake_lock.unlock_fd, WAKE_LOCK_STRING, wake_lock.write_size);
	if(write_res == wake_lock.write_size)
