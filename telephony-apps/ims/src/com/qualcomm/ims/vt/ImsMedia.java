...
class ImsMedia extends Handler {
    ...
    // Holds the latest player state event received so that it can be
    // posted to UI after a conference merge.
    private int mPlayerStateEvent = PLAYER_STOP_EVENT;
    ...
    /* package */void deInit() {
        ...
        mPlayerStateEvent = PLAYER_STOP_EVENT;
    }
    ...
    private void doOnCachedMediaEvent() {
       ...
       synchronized(this) {
           ...
           // Post the latest player state and peer resolution.
           postLatestMediaState();
           mShouldCacheMediaEvents = false;
           mCachedMediaEvents.clear();
       }
    }

    private void postLatestMediaState() {
        log("postLatestMediaState: playerstate = " + mPlayerStateEvent);
        doOnMediaEvent(mPlayerStateEvent);
        doOnMediaEvent(PEER_RESOLUTION_CHANGE_EVT);
    }
    ...
    public void setPreviewImage(Bitmap bitmap) {
        ...
        log("setPreviewImage: bitmap = " + bitmap + " mNegotiatedWidth = " + mNegotiatedWidth +
                " mNegotiatedHeight = " + mNegotiatedHeight);

        int status;
        if (bitmap == null) {
            status = nativeSetVideoImageBuffer(null, mNegotiatedWidth,
                    mNegotiatedHeight);
        } else {
            int [] argb = new int[mNegotiatedWidth * mNegotiatedHeight];
            if (bitmap != null) {
                // argb contains bitmap colors
                bitmap.getPixels(argb, 0, mNegotiatedWidth, 0, 0, mNegotiatedWidth,
                        mNegotiatedHeight);
            }
            status = nativeSetVideoImageBuffer(argb, mNegotiatedWidth,
                    mNegotiatedHeight);
        }
    ...
    private void doOnMediaEvent(int eventId) {
        switch (eventId) {
            ...
            case PLAYER_START_EVENT:
                ...
                mPlayerStateEvent = PLAYER_START_EVENT;
                if (mMediaListener != null) {
    ...
    public void onMediaEvent(int eventId) {
        ...
        synchronized(this) {
            ...
            if (mShouldCacheMediaEvents && isCachingNeededForEvent(eventId)) {
    ...
    // For some events we will simply send the latest state instead of caching
    private boolean isCachingNeededForEvent(int eventId) {
        return !(eventId == START_READY_EVT || eventId == STOP_READY_EVT
                || eventId == PLAYER_START_EVENT || eventId == PLAYER_STOP_EVENT
                || eventId == PEER_RESOLUTION_CHANGE_EVT);
    }
