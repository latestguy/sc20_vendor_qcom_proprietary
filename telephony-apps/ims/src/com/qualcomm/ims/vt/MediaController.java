...
public class MediaController implements ImsMedia.IMediaListener, ICallListListener {
    ...
    private void setVideoImageAsync(String uri) {
        final AsyncTask<Void, Void, Bitmap> task = new AsyncTask<Void, Void, Bitmap>() {
            // Decode image in background.
            @Override
            protected Bitmap doInBackground(Void... params) {
                int negotiatedWidth = mMedia.getNegotiatedWidth();
                int negotiatedHeight = mMedia.getNegotiatedHeight();
                log("setVideoImageAsync: negotiatedWidth = " + negotiatedWidth +
                        " negotiatedHeight = " + negotiatedHeight + " uri = " + uri);

                if (uri.isEmpty()) {
                    // uri is empty, so fall back to default image
                    return QtiImsExtUtils.decodeImage(mContext.getResources(),
                            R.drawable.img_no_image, negotiatedWidth, negotiatedHeight);
                }
                return QtiImsExtUtils.decodeImage(uri, negotiatedWidth, negotiatedHeight);
            }

            @Override
            protected void onPostExecute(Bitmap bitmap) {
                log("setVideoImageAsync bitmap = " + bitmap);
                mMedia.setPreviewImage(bitmap);
            }
        };
        task.execute();
    }
    ...
    public void setPauseImage(String uri) {
        if (uri == null) {
            ...
            mMedia.setPreviewImage(null);
            return;
        }
        setVideoImageAsync(uri);
    }
    ...
    @Override
    public void onMultiPartyStateChanged(ImsCallSessionImpl callSession, boolean isMpty) {
        log("onMultiPartyStateChanged callsession=" + callSession + " isMpty = " + isMpty);
        if (isMpty) mMedia.updateMergeStatus(ImsCallUtils.SESSION_MERGE_COMPLETED);
    }
