...
public class QcrilMsgTunnelSocket {
            ...
            switch (msg.what) {
                case EVENT_SEND:
                    ...
                    try {
                        ...
                        logd("writing packet: " + data.length + " bytes");
                ...
                for (;;) {
                    ...
                    try {
                        ...
                        for (;;) {
                            ...
                            logd("Read packet: " + length +
                                    " bytes. Data Available = " + p.dataAvail() +
                                    " Position = " + p.dataPosition());
    ...
    private static int readRilMessage(InputStream is, byte[] buffer)
        ...
        remaining = 4;
        countRead = 0;
        logd("Before reading offset = " + offset + " remaining = " + remaining +
                " countRead = " + countRead);
        do {
            countRead = is.read(buffer, offset, remaining);
            logd("After reading offset = " + offset + " remaining = "
        ...
        logd("messageLength extracted from first 4 inputStream reads = " +
                messageLength);
        ...
        countRead = 0;
        logd("offset = " + offset + " remaining = " + remaining +
                " countRead = " + countRead);
        do {
            countRead = is.read(buffer, offset, remaining);
            logd("offset = " + offset + " remaining = " + remaining +
        ...
        logd("readRilMessage: Buffer = " + buffer + " HexData = ["
    ...
    private void clearRequestsList(int error) {
        ...
        synchronized (mRequestsList) {
            int count = mRequestsList.size();
            logd("WAKE_LOCK_TIMEOUT " + " mReqPending=" +
                    mRequestMessagesPending + " mRequestList=" + count);
            for (int i = 0; i < count; i++) {
                rr = mRequestsList.get(i);
                logd(i + ": [" + rr.mSerial + "] " +
                        requestToString(rr.mRequest));
    ...
    private Object responseRaw(Parcel p) {
        ...
        if (response != null) {
            logd("ByteArray from parcel = " + bytesToHexString(response,
                            response.length));
        }
    ...
    private void processResponse(Parcel p) {
        ...
        if (type == RESPONSE_UNSOLICITED) {
            logd("Rcvd UNSOL response with " + p.dataAvail() +
                    " bytes data for SUB" + mInstanceId);

            processUnsolicited(p);
        } else if (type == RESPONSE_SOLICITED) {
            logd("Rcvd SOLICITED response with " + p.dataAvail() +
                    " bytes data for SUB" + mInstanceId);
            processSolicited(p);
        } else {
    ...
    private void processSolicited(Parcel p) {
        ...
        logd("processSolicited. sno: " + serial + " error: " + error);
        logd("p.dataAvail():" + p.dataAvail() + " Position():" +
                p.dataPosition());
