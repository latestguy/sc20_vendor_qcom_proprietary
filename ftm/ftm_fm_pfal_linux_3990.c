...
{
    ALOGD("Entered %s", __func__);
    struct RDSCntrsParams  *responce = (struct RDSCntrsParams *)rds_grp_cntr_buff;
    ALOGI("start copy RDS grp counter extended  response ");
    memcpy((void*)&fm_global_params.rds_group_counters,(void*)responce,RDS_GRP_CNTRS_SIZE);
    ALOGD("Exit %s", __func__);
}
void rds_grp_cntrs_ext_rsp_cb(char * rds_grp_cntr_buff)
{
    ALOGD("rds_grp_cntrs_ext_rsp_cb");
    struct RDSCntrsExtParams  *response = (struct RDSCntrsExtParams*)rds_grp_cntr_buff;
    ALOGI("start copy RDS grp counter response ");
    memcpy((void*)&fm_global_params.rds_group_counters_extended,(void*)response,RDS_GRP_CNTRS_SIZE);
    ALOGD("Exit %s", __func__);
}
...
)
{
  ...
  configparams->mute_status = vendor_interface->get_fm_ctrl(HCI_FM_HELIUM_AUDIO_MUTE, value);
  if (configparams->mute_status < 0) {
