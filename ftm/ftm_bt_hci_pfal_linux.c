...
request_status ftm_bt_hci_pfal_nwrite(uint8 *buf, int size)
{
  ...
  uint8 loop_back_cmd[HCI_LB_CMD_SIZE] = {0x1, 0x02, 0x18, 0x01, 0x01};
  ...
  {
     is_slim_bus_test = 1;
     size = HCI_LB_CMD_SIZE;
     buf_size = size; // updating size to loopback command packet size
     printf("\nPinConnectivityTest: Sending loopback command to SOC before initiasing slimbus\n");
     memcpy((void *)buf, (void *)loop_back_cmd, (size_t)size);
  }
  do
  {
