...
dont_read_pkt_ind:
        ...
        while (n_total) {
            if((n_bytes = write(dst_fd, buf_in + len, n_total)) >= 0) {
                ...
                ERROR("%s: unable to write to pc_if fd = %d err = %s", __func__, dst_fd, strerror(errno));
                break;
            }
        }
        if (n_total)    //error
...
static int port_init_libbt(uint8 option)
{
   ...
   if (option == MODE_ANT_UART)  opCmd = BT_VND_OP_ANT_USERIAL_OPEN;
   ...
   if (p_btf->op(BT_VND_OP_POWER_CTRL, &iState) < 0){
      ERROR("bt power on failed \n");
      return -1;
   }

   if (p_btf->op(opCmd, (void*)fd_array) < 0){
      ERROR("bt op(BT_VND_OP_USERIAL_OPEN) failed \n");
      return -1;
   }
