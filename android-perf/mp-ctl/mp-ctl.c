...
static int min_core_lock_supported = 1;   //1 represent min_core lock is supported, 0 not supported
...
static int minimum_online_cores_required = -1; // initialized for each chipset based on their requirement
 ...
 */
static inline int get_online_cpus (int * status_array, int array_sz) {
    int start = 0;
    int end = 0;
    int range = 0;
    int i = 0;
...
static int update_core_based_param(int iscorebased, char node[NODE_MAX], char node_core[NODE_MAX], \
                                   char node_strg[NODE_MAX], int node_strg_l, char *str) {
    int rc = -1;
    int i;
...
#ifdef PERFD
    if (core_ctl_present > 0 && min_core_lock_supported) {
        int rc = -1;
...
static int lock_max_cores(uint16_t opt_data)
{
    ...
    if (core_ctl_present > 0) {
       ...
       if(minimum_online_cores_required != -1) {
          if ((0xF-reqval) < minimum_online_cores_required) {
            return FAILED;
          }
       } else {
          QLOGE("Warning: Minimum cores required to be online is not initialized");
          return FAILED;
       }
...
static int get_online_core(int cpu, int max_num_cpu_clustr)
{
    int i;
    get_online_cpus(core_status, sizeof(core_status)/sizeof(core_status[0]));
    for (i = cpu; i < max_num_cpu_clustr; i++) {
         if (get_core_status(i))
             return i;
    }
    return FAILED;
}
...
{
   int i, clust_id, clust_cpu_num;
   int is_success = 0;

   if (cpu >= 0 && cpu < max_num_cpu_clustr_0) {
       clust_id = 0;
       clust_cpu_num = max_num_cpu_clustr_0;
   } else {
       clust_id = max_num_cpu_clustr_0;
       clust_cpu_num = max_num_cpu_clustr_1;
   }

   for (cpu = clust_id; cpu < clust_cpu_num; cpu++) {
...
static int interactive_pro_above_hispeed_delay(uint16_t data)
{
    ...
    if (reqtype == CPU0_INTERACTIVE_ABOVE_HISPEED_DELAY) {
        iahd[27] = get_online_core(0, max_num_cpu_clustr_0) + '0';
        ipahd_s = ip_0_ahd_s;
        ipahd_sl = &ip_0_ahd_sl;
    } else {
        iahd[27] = get_online_core(max_num_cpu_clustr_0, max_num_cpu_clustr_1) + '0';
        ipahd_s = ip_4_ahd_s;
...
static int interactive_pro_boost(uint16_t data)
{
    ...
    if (reqtype == CPU0_INTERACTIVE_BOOST) {
        ib[27] = get_online_core(0, max_num_cpu_clustr_0) + '0';
        ipb_s = ip_0_b_s;
        ipb_sl = &ip_0_b_sl;
    } else {
        ib[27] = get_online_core(max_num_cpu_clustr_0, max_num_cpu_clustr_1) + '0';
        ipb_s = ip_4_b_s;
        ipb_sl = &ip_4_b_sl;
    }
...
static int interactive_pro_boostpulse(uint16_t data)
{
    ...
    if (reqtype == CPU0_INTERACTIVE_BOOSTPULSE) {
        ibp[27] = get_online_core(0, max_num_cpu_clustr_0) + '0';
    } else {
        ibp[27] = get_online_core(max_num_cpu_clustr_0, max_num_cpu_clustr_1) + '0';
    }
...
static int interactive_pro_boostpulse_duration(uint16_t data)
{
    ...
    if (reqtype == CPU0_INTERACTIVE_BOOSTPULSE_DURATION) {
        ibpd[27] = get_online_core(0, max_num_cpu_clustr_0) + '0';
        ipbpd_s = ip_0_bpd_s;
        ipbpd_sl = &ip_0_bpd_sl;
    } else {
        ibpd[27] = get_online_core(max_num_cpu_clustr_0, max_num_cpu_clustr_1) + '0';
        ipbpd_s = ip_4_bpd_s;
...
static int interactive_pro_go_hispeed_load(uint16_t data)
{
    ...
    if (reqtype == CPU0_INTERACTIVE_GO_HISPEED_LOAD) {
        ighl[27] = get_online_core(0, max_num_cpu_clustr_0) + '0';
        ipghl_s = ip_0_ghl_s;
        ipghl_sl = &ip_0_ghl_sl;
    } else {
        ighl[27] = get_online_core(max_num_cpu_clustr_0, max_num_cpu_clustr_1) + '0';
        ipghl_s = ip_4_ghl_s;
...
static int interactive_pro_hispeed_freq(uint16_t data)
{
    ...
    if (reqtype == CPU0_INTERACTIVE_HISPEED_FREQ) {
        ihf[27] = get_online_core(0, max_num_cpu_clustr_0) + '0';
        iphf_s = ip_0_hf_s;
        iphf_sl = &ip_0_hf_sl;
    } else {
        ihf[27] = get_online_core(max_num_cpu_clustr_0, max_num_cpu_clustr_1) + '0';
        iphf_s = ip_4_hf_s;
        iphf_sl = &ip_4_hf_sl;
    }
...
static int interactive_pro_io_is_busy(uint16_t data)
{
    ...
    if (reqtype == CPU0_INTERACTIVE_IO_IS_BUSY) {
        iioib[27] = get_online_core(0, max_num_cpu_clustr_0) + '0';
        ipioib_s = ip_0_ioib_s;
        ipioib_sl = &ip_0_ioib_sl;
    } else {
        iioib[27] = get_online_core(max_num_cpu_clustr_0, max_num_cpu_clustr_1) + '0';
...
static int interactive_pro_min_sample_time(uint16_t data)
{
    ...
    if (reqtype == CPU0_INTERACTIVE_MIN_SAMPLE_TIME) {
        imst[27] = get_online_core(0, max_num_cpu_clustr_0) + '0';
        ipmst_s = ip_0_mst_s;
        ipmst_sl = &ip_0_mst_sl;
    } else {
        imst[27] = get_online_core(max_num_cpu_clustr_0, max_num_cpu_clustr_1) + '0';
        ipmst_s = ip_4_mst_s;
...
static int interactive_pro_target_loads(uint16_t data)
{
    ...
    if (reqtype == CPU0_INTERACTIVE_TARGET_LOADS) {
        itl[27] = get_online_core(0, max_num_cpu_clustr_0) + '0';
        iptl_s = ip_0_tl_s;
        iptl_sl = &ip_0_tl_sl;
    } else {
        itl[27] = get_online_core(max_num_cpu_clustr_0, max_num_cpu_clustr_1) + '0';
        iptl_s = ip_4_tl_s;
        iptl_sl = &ip_4_tl_sl;
    }
...
static int interactive_pro_timer_rate(uint16_t data)
{
    ...
    if (reqtype == CPU0_INTERACTIVE_TIMER_RATE) {
        itrate[27] = get_online_core(0, max_num_cpu_clustr_0) + '0';
        iptr_s = ip_0_tr_s;
        iptr_sl = &ip_0_tr_sl;
    } else {
        itrate[27] = get_online_core(max_num_cpu_clustr_0, max_num_cpu_clustr_1) + '0';
        iptr_s = ip_4_tr_s;
        iptr_sl = &ip_4_tr_sl;
    }
...
static int interactive_pro_timer_slack(uint16_t data)
{
    ...
    if (reqtype == CPU0_INTERACTIVE_TIMER_SLACK) {
        its[27] = get_online_core(0, max_num_cpu_clustr_0) + '0';
        ipts_s = ip_0_ts_s;
        ipts_sl = &ip_0_ts_sl;
    } else {
        its[27] = get_online_core(max_num_cpu_clustr_0, max_num_cpu_clustr_1) + '0';
        ipts_s = ip_4_ts_s;
        ipts_sl = &ip_4_ts_sl;
    }
...
void del_req_list()
{
    ...
    while (current != NULL)
    {
        if (current->handle->duration != INDEFINITE_DURATION) {
            rc = timer_delete(current->handle->timer);
            if (rc != 0) {
...
int internal_perf_lock_acq(int handle, int duration, int list[], int num_args, pid_t client_pid, pid_t client_tid)
{
    ...
    if (exists != NULL) {
        ...
        if (exists->duration != INDEFINITE_DURATION)
            set_timer(exists);
    ...
    if (duration != INDEFINITE_DURATION) {
        timer_rc = create_timer(req, handle_counter);
        if (timer_rc != 0) {
            free(req);
    ...
    if (req->duration != INDEFINITE_DURATION) {
        set_timer(req); // To reach here, timer create has succeeded
    }
...
static void initialize_target()
{
  if (is_msm8939(get_soc_id()))
  {
      ...
      core_ctl_cpu = 0;
      minimum_online_cores_required = 0;
  }
  ...
  else if (is_msm8916(get_soc_id()))
  {
      ...
      core_ctl_cpu = 0;
      min_core_lock_supported = 0;
      minimum_online_cores_required = 1;
  }
...
#   define ITS(x)          its[cpu_prefix_len+x]
    ...
    for (;;) {
        ...
        clock_gettime(CLOCK_MONOTONIC, &connected);
        //TODO: sizes bigger than mpctl_msg_t structure to be handled
