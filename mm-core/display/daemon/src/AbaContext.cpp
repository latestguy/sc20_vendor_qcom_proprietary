...
static int32_t outCmap(int32_t fb, uint32_t *outcmap)
{
    ...
    case 256:
        ...
        for (i = 0; (uint32_t)i < BL_LUT_SIZE; i++)
            cmap_data[i] = outcmap[i] &
                                    ((1 << gHwInfo.nPriDisplayLutBitDepth) -1);
...
void AbaContext::initHW() {
    ...
    mAbaHwInfo.uLUTSizeBitDepth = BL_LUT_SIZE_BIT_DEPTH;
    mAbaHwInfo.uFactor = HIST_PROC_FACTOR;
    mAbaHwInfo.uHistogramBitDepth = ABA_HISTOGRAM_BIT_DEPTH;
    mAbaHwInfo.uLUTMaxVal = gHwInfo.nPriDisplayLutMaxVal;
    mAbaHwInfo.uLUTBitDepth = gHwInfo.nPriDisplayLutBitDepth;
