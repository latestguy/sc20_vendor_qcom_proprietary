...
int32_t CP::GetElementUnits(const char * name) {
    ...
    str_attr = (char*) xmlEl->Attribute(UNITS_ATTRIBUTE);

    if (str_attr) {
        if (strncmp(str_attr, "int", 3) == 0)
            return UNITS_TYPE_INT;
        if (strncmp(str_attr, "uint", 4) == 0)
            return UNITS_TYPE_UINT;
        if (strncmp(str_attr, "long", 4) == 0)
            return UNITS_TYPE_LONG;
        if (strncmp(str_attr, "double", 6) == 0)
...
int32_t CP::GetGroupID(char *gpID){
    ...
    str = (char*) xmlRoot->Attribute("id");
    if (!str) {
        return ERROR_ELEMENT_NOT_FOUND;
    } else {
        n = snprintf(gpID, MAX_VARIABLE_NAME_LENGTH, "%s", str);
...
int32_t CP::ParseAsArray(const char* name, int32_t type, T **v, int32_t len){
    ...
    char *text, *pCh, *saveptr, *str_text(NULL);
    ...
    str_text = (char*) xmlEl->GetText();

    if (!str_text) {
        return ERROR_ELEMENT_NOT_FOUND;
    } else {
        n = snprintf(text, MAX_ARRAY_STRING_LENGTH, "%s", str_text);
        if (n <= 0)
...
int32_t CABLHWParams::ParseNonArrayParameters() {
    ...
    if (ret && ret != ERROR_ELEMENT_NOT_FOUND)
        return ret;
    mBLMaxLevel = tmp > 0 ? tmp : MDSS_MAX_BL_BRIGHTNESS;
...
void CABLHWParams::LoadDefaults() {
    ...
    uint32_t BLMinRatio[]          = {  512,  665,  768};
    uint32_t BLMaxRatio[]          = {  950,  950,  950};
    uint32_t PixelDistortion[]     = {  100,  100,  100};
    uint32_t BLChangeSpeed[]       = {    2,    2,    2};

    double   RPSoftClippingSlope[]                = { 0.32, 0.28, 0.25};
    int32_t  RPLUTType[]                          = {    3,    3,    3};
    uint32_t RPWindowSizeThreshold[]              = {    8,    6,    4};
    uint32_t RPFilterCoefficientThreshold[]       = {  820,  750,  600};
    uint32_t RPBackLightReductionFactor[]         = {    1,    1,    1};
    uint32_t RPBackLightStepSizeHighCorrelation[] = {    2,    2,    2};
    uint32_t RPSceneCorrelationThreshold[]        = { 1024, 1024, 1024};
