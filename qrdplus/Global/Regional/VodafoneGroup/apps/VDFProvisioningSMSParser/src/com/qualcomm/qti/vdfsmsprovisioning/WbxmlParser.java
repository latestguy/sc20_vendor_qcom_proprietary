...
public class WbxmlParser implements XmlPullParser {
    ...
    public String getPositionDescription() {
        StringBuffer localStringBuffer = new StringBuffer(
                this.mType < XmlPullParser.TYPES.length
                        ? XmlPullParser.TYPES[this.mType] : "unknown");
        ...
        if ((this.mType == 2) || (this.mType == 3)) {
            ...
            for (int j = 0; j < i; j += 4) {
                ...
                if (this.mAttributes[(j + 1)] != null)
                    localStringBuffer.append("{"
                            + this.mAttributes[j] + "}"+ this.mAttributes[(j + 1)] + ":");
    ...
    public String getAttributeValue(String paramString1, String paramString2) {
        for (int i = (this.mAttributeCount << 2) - 4; i >= 0; i -= 4) {
            if ((this.mAttributes[(i + 2)].equals(paramString2))
                    && ((paramString1 == null)
                            || (this.mAttributes[i].equals(paramString1))))
    ...
    public void require(int paramInt, String paramString1, String paramString2)
        ...
        if ((paramInt != this.mType)
              || ((paramString1 != null)
                    && (!paramString1.equals(getNamespace())))
              || ((paramString2 != null)
                    && (!paramString2.equals(getName()))))
            exception("expected: " + (paramInt == 64 ? "WAP Ext."
                    : new StringBuffer().append(XmlPullParser.TYPES[paramInt])
                            .append(" {").append(paramString1)
                            .append("}").append(paramString2).toString()));
    }
    ...
    public void setProperty(String paramString, Object paramObject)
        ...
        throw new XmlPullParserException(
                "unsupported property: " + paramString);
    }
            ...
            for (int i = (this.mAttributeCount << 2) - 4; i >= 0; i -= 4) {
                ...
                if (j == 0)
                    throw new RuntimeException(
                            "illegal attribute mName: " + str1 + " at " + this);
                if (j != -1) {
                    str2 = str1.substring(0, j);
                    str1 = str1.substring(j + 1);
                    ...
                    if (str3 == null)
                        throw new RuntimeException(
                                "Undefined Prefix: " + str2 + " mIn " + this);
    ...
    private final void setTable(int paramInt1, int paramInt2,
        ...
        this.mTables.setElementAt(paramArrayOfString,
                paramInt1 * 3 + paramInt2);
    }
    ...
    private void selectPage(int paramInt, boolean paramBoolean)
        ...
        if (paramBoolean) {
            this.mTagTable = ((String[]) this.mTables
                    .elementAt(paramInt * 3 + this.TAG_TABLE));
        } else {
            this.mAttrStartTable = ((String[]) this.mTables
                    .elementAt(paramInt * 3 + this.ATTR_START_TABLE));
            this.mAttrValueTable = ((String[]) this.mTables
                    .elementAt(paramInt * 3 + this.ATTR_VALUE_TABLE));
        }
    }
        ...
        while (i != 1) {
            ...
            for (i = readByte(); (i > 128) || (i == 0) || (i == 2) || (i == 3)
                ...
                switch (i) {
                    ...
                    case 195:
                        localStringBuffer.append(
                                resolveWapExtension(i, parseWapExtension(i)));
                        break;
                    case 131:
                    ...
                    default:
                        localStringBuffer.append(
                                resolveId(this.mAttrValueTable, i));
                }
    ...
    protected String resolveWapExtension(int paramInt, Object paramObject) {
        if ((paramObject instanceof byte[])) {
            ...
            for (int i = 0; i < arrayOfByte.length; i++) {
                localStringBuffer.append(
                        "0123456789abcdef".charAt(arrayOfByte[i] >> 4 & 0xF));
                localStringBuffer.append(
                        "0123456789abcdef".charAt(arrayOfByte[i] & 0xF));
            }
            return localStringBuffer.toString();
        }
        return "$(" + paramObject + ")";
    ...
    /* This line to be called in a loop to convert a number stored
     * as a collection of 7-bit values back to an int
     * "j & 0x7F" will grab 7 bits of data from inputstream.
     * "i << 7" will shift number 'i' to left by 7 places.
     */
