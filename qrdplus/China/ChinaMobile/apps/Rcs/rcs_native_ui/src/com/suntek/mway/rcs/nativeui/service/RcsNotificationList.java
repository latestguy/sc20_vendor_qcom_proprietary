...
public class RcsNotificationList extends ArrayList<RcsNotification> {
     ...
     */
    public void removeInviteNotificationByChatUri(long groupId) {
        if (groupId <= 0) {
            return;
        }
        ...
        for (RcsNotification notification : this) {
            if (notification instanceof RcsGroupChatInviteNotification) {
                if (groupId == ((RcsGroupChatInviteNotification) notification).getGroupId()) {
    ...
    public boolean isDuplicateInviteNotification(long groupId) {
        if (groupId <= 0) {
            return true;
        }
