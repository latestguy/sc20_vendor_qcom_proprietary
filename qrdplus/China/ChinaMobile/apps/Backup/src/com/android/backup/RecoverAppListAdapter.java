...
public class RecoverAppListAdapter extends BaseAdapter
{
    ...
    public void sortImpl() {
        ...
        for (String p : mFileNameList) {
            ...
            if (needInitChecked) mFileCheckedList.add(Boolean.FALSE);
        }
    }
    ...
    public void operateMultiRestore(RecoverAppActivity activity) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (filename != null) {
                try {
                    ...
                    File apkFile = new File(mBackupRootDir + "/" + filename);
                    if (apkFile.isFile() && apkFile.getName().endsWith(".apk")) {
                        installOnBackground(apkFile);
                    }
                    /**
        ...
        //Log.d(TAG, "sendbroadcast:RECOVERAPPDATA");
        if (!activity.mCancel)
        {
            BackupUtils.writePathToFile(mContext.getFilesDir().getPath() +
                    BackupUtils.RESTORE_PATH_FILE, mRecoverPath);
            /** Per android M SELinux MLS, can not run backupAppData.sh normal,
                so don't need recoverAppData.sh
            activity.sendBroadcast(new Intent("android.intent.action.RECOVERAPPDATA"));
            Log.d(TAG, "sendbroadcast:RECOVERAPPDATA end");
    ...
    private void installOnBackground(File file) {
        ...
        pm.installPackage(packageUri, observer, installFlags, "com.android.backupreceiver");
    }
