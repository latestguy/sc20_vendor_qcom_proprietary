...
public class VcalendarImporter {
    ...
    public final int insertFromFile(File file) {
        ...
        try {
            ...
            String vTodoString = "";
            String parseString = "";
            ArrayList<VCalendarManager> vCalendarManagerList = new ArrayList<VCalendarManager>();
                        ...
                        if (inVevent)
                        {
                            ...
                            if (name.toUpperCase().trim().equals(END))
                            {
                                ...
                                if (vcalName.toUpperCase().trim().equals(VEVENT))
                                {
                                    ...
                                    VCalendarManager vCalManager = new VCalendarManager(mContext, parseString);
                                    vCalendarManagerList.add(vCalManager);
                                    vEventString = "";
                                }
                                else
                                {
            ...
            int size = 0;
            for (VCalendarManager vcm : vCalendarManagerList) {
                size += vcm.getmVCalValuesListSize();
            }
            mCallBackHandler.sendMessage(mCallBackHandler.obtainMessage(EVENT_INIT_PROGRESS_TITLE,
                    BACKUP_TYPE_CALENDAR, size));
            for (VCalendarManager vcm : vCalendarManagerList) {
                int result = vcm.saveVcalendar_new(fullResult);
                if (result > 0)
                {
                    insertIndex += result;
