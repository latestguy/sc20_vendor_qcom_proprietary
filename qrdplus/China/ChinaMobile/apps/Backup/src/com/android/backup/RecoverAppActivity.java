...
public class RecoverAppActivity extends ListActivity implements View.OnClickListener {
    ...
    private HashMap<String,Boolean> mStoreStatusMap = null;
    ...
    private void initData(){
        ...
        if ( mRestoreBundle != null ) {
            Log.d(TAG, "initData go Restore process: the activity may re-create");
            restoreCheckedStatus((HashMap)mRestoreBundle.getSerializable("storeData"));
            mRestoreBundle = null;
        } else if ( mStoreStatusMap != null ) {
            Log.d(TAG, "initData go store process: the activity may re-launcher");
            restoreCheckedStatus(mStoreStatusMap);
            mStoreStatusMap = null;
        } else {
            Log.d(TAG, "initData go clean process: the activity may clean create");
    ...
    private HashMap getStoreData() {
        ...
        ArrayList fileNameList = mOnlyAdapter.getFileNameList();
        ArrayList checkedList = mOnlyAdapter.getFileCheckedList();
        int length = fileNameList.size();

        for ( int i = 0; i < length; i++){
            Log.d(TAG, " store apkName: " + fileNameList.get(i) + " checked:" + (boolean)checkedList.get(i));
            map.put((String)fileNameList.get(i),(boolean)checkedList.get(i));
        }
    ...
    public void onClick(View v) {
        if (v instanceof Button) {
            if (!BackupUtils.isDirAvalible(mBackupRootDir)) {
                Log.d(TAG, "Recover path don't exist");
                Toast.makeText(getApplicationContext(), R.string.restore_path_dont_exist, Toast.LENGTH_SHORT)
                        .show();
                return;
            }
    ...
    private void restoreCheckedStatus(HashMap<String,Boolean> map) {
        ...
        for ( int i = 0; i < length; i++){
            if (!map.containsKey(fileNameList.get(i))) continue;
            Log.d(TAG, "restore apkName: " + fileNameList.get(i) + "checked: " + (boolean)map.get(fileNameList.get(i)));
            checkedList.set(i,(boolean)map.get(fileNameList.get(i)));
        }

        mOnlyAdapter.setFileCheckedList(checkedList);
        if (mOnlyAdapter.getSelectCnt() > 0)
            mRestoreBtn.setEnabled(true);
        else
            mRestoreBtn.setEnabled(false);
