...
static IU8* dmTreeCopyString(IU8* buf, const IS8* str) {
    *buf = (IU8) (strlen(str) + 1);
    strlcpy((IS8*) (buf + 1), str, strlen(str) + 1);
    buf += (*buf) + 1;
    return buf;
}
...
static IU8* dmTreeGetString(IU8* buf, IS8* str) {
    IS32 len = *buf++;
    strlcpy(str, (IS8*) buf, strlen(buf) + 1);
    buf += len;
    return buf;
}
...
DMTREE_ACLSVRLIST* dmTreeCopyAclServerList(DMTREE_ACLSVRLIST *lpAclList) {
    ...
    while (addAcl != NULL ) {
         ...
         *   copy acl value
         */
        strlcpy(newAcl->serverID, addAcl->serverID, sizeof(newAcl->serverID));
...
DMTREE_INTERNAL_ITEM* dmTreeInternal_insertItem(
    ...
    if (newItem->staticData) {
        ...
        if (newItem->nodeData != NULL ) {
            ...
            strlcpy(lpNewItem->data->nodeData, newItem->nodeData,
        ...
        if (newItem->nodeUri[0] == '.') {
            strlcpy(lpNewItem->data->nodeUri, newItem->nodeUri, sizeof(lpNewItem->data->nodeUri));
        } else if (newItem->nodeUri[0] != '\0') {
            int newLen;
            if (parentHndl != NULL) {
                strlcpy(lpNewItem->data->nodeUri, parentHndl->data->nodeUri,
                        sizeof(lpNewItem->data->nodeUri));
            }
            ...
            strlcat(lpNewItem->data->nodeUri, "/", sizeof(lpNewItem->data->nodeUri));
            newLen+=1;
            if ((tmpUri = strrchr(newItem->nodeUri, '/')) != NULL ){
                ...
                strlcat(lpNewItem->data->nodeUri, tmpUri + 1, sizeof(lpNewItem->data->nodeUri));
            }
            else{
                ...
                strlcat(lpNewItem->data->nodeUri, newItem->nodeUri,
                        sizeof(lpNewItem->data->nodeUri));
           }
        } else {
            ...
            strlcat(lpNewItem->data->nodeUri, newItem->nodeName, sizeof(lpNewItem->data->nodeUri));
        }

        /*
         *  Set TreeItem value
         */
        strlcpy(lpNewItem->data->nodeName, newItem->nodeName, sizeof(lpNewItem->data->nodeName));
        strlcpy(lpNewItem->data->nodeType, newItem->nodeType, sizeof(lpNewItem->data->nodeType));
        strlcpy(lpNewItem->data->nodeFormat, newItem->nodeFormat,
                sizeof(lpNewItem->data->nodeFormat));
...
IS32 dmTreeInternal_replaceItem(DMTREE_INTERNAL_ITEM* replaceHndl,
    ...
    if (replaceMask & DMTREE_REPLACE_NODENAME) {
         ...
         *  Replace node name
         */
        strlcpy(replaceHndl->data->nodeName, replaceItem->nodeName,
                sizeof(replaceHndl->data->nodeName));
    }
        ...
        if (replaceItem->nodeData != NULL ) {
            if (replaceHndl->data->replaceFunc != NULL ) {
                ...
                if (replaceHndl->data->dataSize > 0) {
                    ...
                    strlcpy(replaceHndl->data->nodeData, replaceItem->nodeData,
                            replaceHndl->data->dataSize + 1);
                }
            }
        }
...
IS32 dmTreeInternal_replaceAcl(DMTREE_INTERNAL_ITEM* lpTreeHndl,
    ...
    while (*dataEndPtr != '\0') {
        ...
        do {
            ...
            strlcpy(serverId, dataBeginPtr, (IU32) (dataEndPtr - dataBeginPtr) + 1);
            ...
            if (strcmp(serverId, "*") == 0) {
                ...
                if (lpTmpAclList != NULL ) {
                    ...
                    strlcpy(lpTmpAclList->serverID, serverId, sizeof(lpTmpAclList->serverID));
                    lpTmpAclList->accType = accType;
...
IS32 dmTreeInternal_readDmAccItem(DM_TREE_HANDLE treeHndl, IU8* buffer,
    ...
    if (strlen(buf) < DMTREE_NAME_MAXSIZE) {
        strlcpy(g_dmTreeTmpDmAcc.accNodeName, buf, sizeof(g_dmTreeTmpDmAcc.accNodeName));
    }
    buf += strlen(buf) + 1;

    // AppID
    if (strlen(buf) < DMTREE_ID_MAXSIZE) {
        strlcpy(g_dmTreeTmpDmAcc.accAppID, buf, sizeof(g_dmTreeTmpDmAcc.accAppID));
    }
    buf += strlen(buf) + 1;
    // ServerID
    if (strlen(buf) < DMTREE_ID_MAXSIZE) {
        strlcpy(g_dmTreeTmpDmAcc.accServerId, buf, sizeof(g_dmTreeTmpDmAcc.accServerId));
    }
    buf += strlen(buf) + 1;
    // Name
    if (strlen(buf) < DMTREE_NAME_MAXSIZE) {
        strlcpy(g_dmTreeTmpDmAcc.accName, buf, sizeof(g_dmTreeTmpDmAcc.accName));
    }
    buf += strlen(buf) + 1;
    // PrefConRef
    if (strlen(buf) < DMTREE_ID_MAXSIZE) {
        strlcpy(g_dmTreeTmpDmAcc.accPrefConRef, buf, sizeof(g_dmTreeTmpDmAcc.accPrefConRef));
    }
    buf += strlen(buf) + 1;
    // ConRef
    if (strlen(buf) < DMTREE_ID_MAXSIZE) {
        strlcpy(g_dmTreeTmpDmAcc.accConRef, buf, sizeof(g_dmTreeTmpDmAcc.accConRef));
    }
    buf += strlen(buf) + 1;

    // Addr
    if (strlen(buf) < DMTREE_URL_MAXSIZE) {
        strlcpy(g_dmTreeTmpDmAcc.accAddr, buf, sizeof(g_dmTreeTmpDmAcc.accAddr));
    }
    buf += strlen(buf) + 1;
    // AddrType
    if (strlen(buf) < 16) {
        strlcpy(g_dmTreeTmpDmAcc.accAddrType, buf, sizeof(g_dmTreeTmpDmAcc.accAddrType));
    }
    buf += strlen(buf) + 1;
    // Prot Number
    ...
    // Server AuthLevel
    strlcpy(g_dmTreeTmpDmAcc.accServerAuthLevel, buf, sizeof(g_dmTreeTmpDmAcc.accServerAuthLevel));
    buf += strlen(buf) + 1;
    // Server AuthType
    g_dmTreeTmpDmAcc.accServerAuthType = (DMTREE_AUTH_TYPE) (*buf++);
    // Server AuthName
    strlcpy(g_dmTreeTmpDmAcc.accServerAuthName, buf, sizeof(g_dmTreeTmpDmAcc.accServerAuthName));
    buf += strlen(buf) + 1;
    // Server Psw
    strlcpy(g_dmTreeTmpDmAcc.accServerPw, buf, sizeof(g_dmTreeTmpDmAcc.accServerPw));
    buf += strlen(buf) + 1;
    // Server Nonce
    strlcpy((IS8*) g_dmTreeTmpDmAcc.accServerNonce, buf, sizeof(g_dmTreeTmpDmAcc.accServerNonce));
    buf += strlen(buf) + 1;
    // Prev Server Nonce
    strlcpy((IS8*) g_dmTreeTmpDmAcc.accPrevServerNonce, buf,
	     sizeof(g_dmTreeTmpDmAcc.accPrevServerNonce));
    buf += strlen(buf) + 1;

    // Client AuthLevel
    strlcpy(g_dmTreeTmpDmAcc.accClientAuthLevel, buf, sizeof(g_dmTreeTmpDmAcc.accClientAuthLevel));
    buf += strlen(buf) + 1;
    // Client AuthType
    g_dmTreeTmpDmAcc.accClientAuthType = (DMTREE_AUTH_TYPE) (*buf++);
    // User Name
    strlcpy(g_dmTreeTmpDmAcc.accUserName, buf, sizeof(g_dmTreeTmpDmAcc.accUserName));
    buf += strlen(buf) + 1;
    // User Psw
    strlcpy(g_dmTreeTmpDmAcc.accUserPw, buf, sizeof(g_dmTreeTmpDmAcc.accUserPw));
    buf += strlen(buf) + 1;
    // Client Nonce
    strlcpy((IS8*) g_dmTreeTmpDmAcc.accClientNonce, buf, sizeof(g_dmTreeTmpDmAcc.accClientNonce));
    buf += strlen(buf) + 1;
...
IBOOL dmTreeInternal_saveDmAccItem(DM_FILE_HANDLE fileHndl,
    ...
    // node name
    strlcpy(buf, dmAccInfo->accNodeName, sizeof(dmAccInfo->accNodeName));
    buf += strlen(buf) + 1;
    // AppID
    strlcpy(buf, dmAccInfo->accAppID, sizeof(dmAccInfo->accAppID));
    buf += strlen(buf) + 1;
    // ServerID
    strlcpy(buf, dmAccInfo->accServerId, sizeof(dmAccInfo->accServerId));
    buf += strlen(buf) + 1;
    // Name
    strlcpy(buf, dmAccInfo->accName, sizeof(dmAccInfo->accName));
    buf += strlen(buf) + 1;
    // PrefConRef
    strlcpy(buf, dmAccInfo->accPrefConRef, sizeof(dmAccInfo->accPrefConRef));
    buf += strlen(buf) + 1;
    // ConRef
    strlcpy(buf, dmAccInfo->accConRef, sizeof(dmAccInfo->accConRef));
    buf += strlen(buf) + 1;

    // Addr
    strlcpy(buf, dmAccInfo->accAddr, sizeof(dmAccInfo->accAddr));
    buf += strlen(buf) + 1;
    // AddrType
    strlcpy(buf, dmAccInfo->accAddrType, sizeof(dmAccInfo->accAddrType));
    buf += strlen(buf) + 1;
    // Prot Number
    ...
    *buf++ = (IS8) dmAccInfo->accAuthPref;
    ...
    // Server AuthLevel
    strlcpy(buf, dmAccInfo->accServerAuthLevel, sizeof(dmAccInfo->accServerAuthLevel));
    buf += strlen(buf) + 1;
    // Server AuthType
    *buf++ = (IS8) dmAccInfo->accServerAuthType;
    // Server AuthName
    strlcpy(buf, dmAccInfo->accServerAuthName, sizeof(dmAccInfo->accServerAuthName));
    buf += strlen(buf) + 1;
    // Server Psw
    strlcpy(buf, dmAccInfo->accServerPw, sizeof(dmAccInfo->accServerPw));
    buf += strlen(buf) + 1;
    // Server Nonce
    strlcpy(buf, (IS8*) dmAccInfo->accServerNonce, sizeof(dmAccInfo->accServerNonce));
    buf += strlen(buf) + 1;
    // Prev Server Nonce
    strlcpy(buf, (IS8*) dmAccInfo->accPrevServerNonce, sizeof(dmAccInfo->accPrevServerNonce));
    buf += strlen(buf) + 1;

    // Client AuthLevel
    strlcpy(buf, dmAccInfo->accClientAuthLevel, sizeof(dmAccInfo->accClientAuthLevel));
    buf += strlen(buf) + 1;
    // Client AuthType
    *buf++ = (IS8) dmAccInfo->accClientAuthType;
    // User Name
    strlcpy(buf, dmAccInfo->accUserName, sizeof(dmAccInfo->accUserName));
    buf += strlen(buf) + 1;
    // User Psw
    strlcpy(buf, dmAccInfo->accUserPw, sizeof(dmAccInfo->accUserPw));
    buf += strlen(buf) + 1;
    // Client Nonce
    strlcpy(buf, (IS8*) dmAccInfo->accClientNonce, sizeof(dmAccInfo->accClientNonce));
    buf += strlen(buf) + 1;
...
DMTREE_INTERNAL_ITEM* dmTreeInternal_createDMAcc(
    ...
    strlcpy(stServerList.serverID, serverID, sizeof(stServerList.serverID));
    ...
    strlcpy(g_dmTreeTmpItem.nodeName, nodeName, sizeof(g_dmTreeTmpItem.nodeName));
    strlcpy(g_dmTreeTmpItem.nodeUri, nodeName, sizeof(g_dmTreeTmpItem.nodeUri));
...
IBOOL dmTreeInternalCB_replaceDMAccServerList(DM_TREE_HANDLE treeHndl,
    ...
    if (lpTreeHndl->data->aclSvrList != NULL )
        strlcpy(lpTreeHndl->data->aclSvrList->serverID, (IS8*) param,
                sizeof(lpTreeHndl->data->aclSvrList->serverID));
