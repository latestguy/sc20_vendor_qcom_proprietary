...
short dm_syncml_startmessage(short id) {
    ...
    if ((dm_task_relay_info->authtype == AUTH_B64)
        ...
        if (0 != xppStrlen(dm_task_relay_info->syncml_dm_password))
            snprintf(creddata, sizeof(creddata), "%s:%s", dm_task_relay_info->syncml_dm_username,
                    dm_task_relay_info->syncml_dm_password);
        else
            snprintf(creddata, sizeof(creddata), "%s", dm_task_relay_info->syncml_dm_username);
        ...
        snprintf(creddata, sizeof(creddata), "%s", dm_task_relay_info->syncml_dm_username);
        xppStrlcat(creddata, ":", sizeof(creddata));
        xppStrlcat(creddata, dm_task_relay_info->syncml_dm_password, sizeof(creddata));
#endif
        ...
        xppStrlcat(creddata, nonce_tem, sizeof(creddata));
        dm_calc_b64_cred(dm_calc_md5_cred(creddata), HASHLEN);
        syncml_core_message("MMIDM $$ dm_syncml_startmessage  creddata=%s ",creddata);
