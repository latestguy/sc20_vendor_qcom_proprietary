...
int verifyCarrierPack(const char* path) {
    ...
    //Certificate* publicKeys = load_keys(PUBLIC_KEYS_FILE, certs);
    bool iSpublicKeys = load_keys(PUBLIC_KEYS_FILE, certs);
    if (!iSpublicKeys) {
        sysReleaseMap(&mmap);
        printf("VerifyCarrierPack: Failed to load public keys\n");
        return 2;
    }
    printf("VerifyCarrierPack:  key(s) loaded from %s\n",
             PUBLIC_KEYS_FILE);

    printf("VerifyCarrierPack: Verifying carrier package...%s\n", path);

    //int error = verify_file(mmap.addr, mmap.length, publicKeys, keysNum);
    int error = verify_file(mmap.addr, mmap.length, certs);
    //free(publicKeys);
