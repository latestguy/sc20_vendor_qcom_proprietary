...
#ifeq (0,1)
ifeq ($(call is-board-platform-in-list, msm8996 msm8916 msm8916_32 msm8916_32_512 msm8916_32_k64 msm8916_32_LMT msm8916_64 msm8916_64_LMT),true)
TARGET_NOT_SUPPORT :=true
endif

ifeq ($(call is-board-platform-in-list, msm8909 msm8909_512 msm8909_LMT msm8909w msm8916 msm8916_32 msm8916_32_512 msm8916_32_k64 msm8916_32_LMT msm8916_64 msm8916_64_LMT),true)
...
$(shell mkdir -p $(TARGET_OUT)/vendor/Default/data/)
$(shell mkdir -p $(TARGET_OUT)/vendor/Default/data/modem_row_config/)
$(shell cp $(LOCAL_PATH)/$(DEFAULT_ROW_MBN) $(TARGET_OUT)/vendor/Default/data/modem_row_config/$(DEFAULT_ROW_MBN))

################################################
SEPOLICY_PATH := "device/qcom/sepolicy"
define is-regionalization-supported
$(shell product=$(1) ; \
  if [ -d "$(SEPOLICY_PATH)/$${product%%_*}" ] ; then \
...
BackupAllCarriers: $(FULL_SYSTEMIMAGE_DEPS)
	...
	@for path in `find $(PRODUCT_OUT)/system/vendor -name ".preloadspec"` ; do \
           ...
           if [ "$$pack" != "" ] ; then \
             ...
             cp -rf $(PRODUCT_OUT)/system/vendor/$$pack $(TARGET_FILES)/temp ;\
           fi ;\
         done
	...
	@touch $(RegionalizationEnvPathSpec)/qgpversion
...
InstallCarrier: $(OriginalBuildPropPath) $(FULL_SYSTEMIMAGE_DEPS) $(InstallCarrierUpdateBinary) $(InstallCarrierSignTool) $(InstallCarrierSignPerm) $(InstallCarrierSignKey) $(TARGET_ROOT_OUT)/fstab.qcom BackupAllCarriers
#InstallCarrier: $(InstallCarrierFileList) $(FULL_SYSTEMIMAGE_DEPS) $(InstallCarrierSignTool) $(InstallCarrierSignPerm) $(InstallCarrierSignKey) $(TARGET_ROOT_OUT)/fstab.qcom BackupAllCarriers
	...
	@if [ "$(USES_REGIONALIZATION_ENVIRONMENT)" == "true" ] ; then \
           temp="$(PRODUCT_OUT)/system/vendor/Default/temp.prop" ;\
           ...
           cat "$(PRODUCT_OUT)/system/vendor/Default/$(BuildPropFileName)" >> "$$temp" ;\
           echo -e "\n" >> "$$temp" ;\
           ...
           mv "$$temp" "$(PRODUCT_OUT)/system/vendor/Default/$(BuildPropFileName)" ;\
         fi
	...
	@if [ "$(USES_REGIONALIZATION_ENVIRONMENT)" == "true" ] ; then \
           ...
           count=0; for pack in $(InstallCarrier_MainPackList) ; do \
             if [ "$${pack//\//}" != "$$pack" ] ; then \
               ...
               packname="$${item[0]}" ;\
               partition="$${item[1]}" ;\
               if [ -d "$(PRODUCT_OUT)/system/vendor/$$packname" ] && [ -d "$(PRODUCT_OUT)/$$partition" ] ; then \
                 cp -rf $(PRODUCT_OUT)/system/vendor/$$packname $(PRODUCT_OUT)/$$partition/ ;\
               ...
               if [ -d "$(PRODUCT_OUT)/system/vendor/$$pack" ] ; then \
                 storagePos="/system/vendor" ;\
           ...
           echo "ro.regionalization.support=$(USES_REGIONALIZATION_ENVIRONMENT)" >> $(TARGET_OUT)/vendor/Default/system/vendor/$(DEFAULT_PROP) ;\
           /bin/bash $(InstallCarrierPathCarrier)/switch_spec.sh $(PRODUCT_OUT)/system/vendor $(PRODUCT_OUT) $(PRODUCT_OUT) $(InstallCarrierPathSpec)/spec Default ;\
           ...
           echo "newPackCount=$$count" > $(PRODUCT_OUT)/system/vendor/speccfg/spec.new ;\
...
clean_outside_packlist:build_oem_carrier_image
	...
	@while read line; do \
           ...
           if [ "$$flag" != "keep" ] ; then \
		     if [ "true" == "false" ] ;  then \
			  echo "packlist:Remove /system/vendor/$$line ..." ;\
			 fi;\
             rm -rf $(PRODUCT_OUT)/system/vendor/$$line ;\
           fi ;\
	...
	@if [ -f "$(TARGET_FILES)/temp/cleanlist" ] ; then \
	   while read line; do \
           ...
           if [ "$$flag" != "keep" ] ; then \
		    if [ "true" == "false" ]; then\
			 echo "cleanlist:Remove /system/vendor/$$line ..." ;\
			fi;\
            rm -rf $(PRODUCT_OUT)/system/vendor/$$line ;\
           fi ;\
