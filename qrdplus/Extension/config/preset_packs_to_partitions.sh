...
CHIPSET_8909=`echo $1 |grep -o 8909`
echo "chipset is $CHIPSET_8909 out=$1 "
...
if [ "$CHIPSET_8909" == "8909" ]
then
PresetPackList4=(TelefonicaGermany/oem H3GUK/oem H3GItalia/oem IndonesiaOpenmarket/oem ThailandOpenMarket/oem NorthAmerica/oem RJIL/oem LatamTelefonica/oem LatamTelefonicaArgentina/oem LatamTelefonicaBrazil/oem LatamTelefonicaChile/oem LatamTelefonicaColombia/oem LatamTelefonicaCostaRica/oem LatamTelefonicaEcuador/oem LatamTelefonicaElSalvador/oem LatamTelefonicaGuatemala/oem LatamTelefonicaMexico/oem LatamTelefonicaNicaragua/oem LatamTelefonicaPanama/oem LatamTelefonicaPeru/oem LatamTelefonicaUruguay/oem LatamTelefonicaVenezuela/oem TelefonicaSpain/oem IndiaCommon/oem EUCommon/oem TelecomItaliaMobile/oem VietnamOpenMarket/oem CherryCommon/oem CherryCambodia/oem CherryLaos/oem CherryMyanmar/oem CherryPhilippines/oem CherryThailand/oem Cambodia/oem Laos/oem LatamBrazil/oem MalaysiaOpenMarket/oem PhilippinesOpenMarket/oem EEEU/oem)
else
...
CPList=( ${PresetPackList1[*]} ${PresetPackList2[*]} ${PresetPackList3[*]} ${PresetPackList4[*]} )
...
adjustPresetPackages()
{
      ...
      then
        ...
        then
          ...
          if [ $partitionSize -gt 0 ] && [ $packsSize -gt $partitionSize ] ;then
            mv -fv $ProductOut/$partition/$pack/data/app/* $ProductOut/data/app-regional/$pack
		  else 
  ...
  then
    packsSize=`du -sb $ProductOut/$partition | cut -f1`
	if [ $packsSize -gt $partitionSize ] ;then
     for path in `find $ProductOut/$partition -name ".preloadspec"`
     do
       echo "$path"
       tmp="${path%\/.preloadspec}"
