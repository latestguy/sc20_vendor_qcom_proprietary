...
public class LunarInfo {
    ...
    public MatrixCursor buildCursor(int year, int month, int day) {
        ...
        DateInfo dateInfo = LunarParser.parseLunarCalendar(null, year, month, day);
        buildCursor(cursor, dateInfo);
        return cursor;
    }
    ...
    public MatrixCursor buildCursor(int from_year, int from_month, int from_day,
        ...
        while (start.getTimeInMillis() <= end.getTimeInMillis()) {
            ...
            dateInfo = LunarParser.parseLunarCalendar(dateInfo, year, month, day);
            buildCursor(cursor, dateInfo);
    ...
    private void buildCursor(MatrixCursor cursor, DateInfo dateInfo) {
        if (cursor == null || dateInfo == null) return;

        // Build the cursor.
        ArrayList<Object> row = new ArrayList<Object>(PROJECTION.length);
        row.add(cursor.getCount());         // _id
        row.add(dateInfo._solar_year);     // year
        row.add(dateInfo._solar_month);    // month
        row.add(dateInfo._solar_day);      // day
        row.add(dateInfo._lunar_year);     // lunar_year
        row.add(dateInfo._lunar_month);    // lunar_month
        row.add(dateInfo._lunar_day);      // lunar_day

        // Get the lunar label.
        String suggestionLongLabel = LunarParser.getLunarLongLabel(mContext, dateInfo);
        row.add(suggestionLongLabel);       // lunar_label_long
        String suggestionShortLabel = LunarParser.getLunarShortLabel(mContext, dateInfo);
        row.add(suggestionShortLabel);      // lunar_label_short

        // Get the animal label.
        String animal = LunarParser.getAnimalsYear(mContext, dateInfo);
        row.add(animal);                    // animal

        // Get the festival and solar terms info.
        ArrayList<String> festivalList = new ArrayList<String>();
        FestivalParser.getFestivals(mContext, dateInfo, festivalList);
        SolarTermsParser.getSolarTerms(mContext, dateInfo, festivalList);
