...
sp<AMessage> DashPlayer::DASHHTTPLiveSource::getTrackInfo(size_t index) {
  return ((mMMIInterface!=NULL) ? mMMIInterface->getTrackInfo(index) : NULL);
}

status_t DashPlayer::DASHHTTPLiveSource::getTrackCount()
{
  return ((mMMIInterface!=NULL) ? mMMIInterface->getTrackCount() : (status_t)UNKNOWN_ERROR);
}
