...
status_t DASHMMIInterface::populateTrackInfo()
{
  ...
  if(bOk)
  {
    ...
    if(err == OK && size > 0)
    {
      ...
      while(bOk && searchAdaptationStr != NULL)
      {
        ...
        //Extract the mime string
        char *searchMimeTypeStr = std_strstr(searchAdaptationStr, "<mimeType");
        //Look for mime string within the particular adaptationSet element
        AString sMimeType;

        if(searchMimeTypeStr != NULL && (nextAdaptationStr == NULL || searchMimeTypeStr < nextAdaptationStr))
        {
          searchMimeTypeStr = std_strchr(searchMimeTypeStr, '>') + 1;
          searchMimeTypeStr = skip_whitespace(searchMimeTypeStr);

          char* endPos = std_strchr(searchMimeTypeStr, '<');

          int len = endPos - searchMimeTypeStr;

          if (len > 0) //mime string found
          {
            sMimeType.append(searchMimeTypeStr, len );
          }
          else //Mime string not found, default implementation
          {
          ...
          MM_MSG_PRIO1(MM_HTTP_STREAMING, MM_PRIO_HIGH,
                "Track Mime string %s", sMimeType.c_str());
        }

        if(bOk && !bFoundVideoCodec && !bFoundAudioCodec && !bFoundTextCodec)
        {
          bOk = addTrackInfoEntry(MEDIA_TRACK_TYPE_UNKNOWN, sMimeType, sLang);
        }
        else
        {
          while(bOk && (bFoundVideoCodec || bFoundAudioCodec || bFoundTextCodec))
          {
            if(bFoundVideoCodec)
            {
              ...
              bOk = addTrackInfoEntry(MEDIA_TRACK_TYPE_VIDEO, sMimeType, sLang);
            }
            else if(bFoundAudioCodec)
            {
              bFoundAudioCodec = false;
              bOk = addTrackInfoEntry(MEDIA_TRACK_TYPE_AUDIO, sMimeType, sLang);
            }
            else if(bFoundTextCodec)
            {
              bFoundTextCodec = false;
              bOk = addTrackInfoEntry(MEDIA_TRACK_TYPE_TIMEDTEXT, sMimeType, sLang);
            }
          }
        }
...
/*
 *  Returns array of trackInfo(tracktype,mime,lang)
 */
sp<AMessage> DASHMMIInterface::getTrackInfo(size_t index)
{
  sp<AMessage> format = new AMessage();
  format->setInt32("type", m_pTrackInfo[index].getTrackType());
  format->setString("language",m_pTrackInfo[index].getLang());
  format->setString("mime", m_pTrackInfo[index].getMimeType());
  return format;
}

/*
 *  returns inband track count
 */
status_t DASHMMIInterface::getTrackCount()
{
   return m_nTotalNumTrackInfo;
}

/*
 *  Update m_pTrackInfo array with new entry
 */
bool DASHMMIInterface::addTrackInfoEntry(media_track_type eTrackType, AString sMimeType, char* sLang)
{
  bool bOk = true;
  ...
  if(bOk)
  {
    ...
    m_pTrackInfo[m_nTotalNumTrackInfo-1].setMimeType(sMimeType);
...
void DASHMMIInterface::sendPrepareDoneCallBack(status_t status)
{
    if (mHttpLiveSrcObj != NULL)
    {
      ...
      if (status == OK)
      {
         status =  populateTrackInfo();
         if(status != OK)
         {
           QTV_MSG_PRIO1(MSG_SSID_APPS_QTV_STREAMING, QTVDIAG_PRIO_FATAL,
             "Error (%d)in populateTrackInfo", status);
         }
      }
    }
}
