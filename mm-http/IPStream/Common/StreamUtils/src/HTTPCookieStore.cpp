                               ...
                               m_pSourceClock(NULL)
{
  ...
  if(!m_pSourceClock || !bOk)
  {
    QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                      "HTTPCookieMgr Error Unable to create source clock" );
    if (m_pSourceClock)
    {
...
HTTPCookieMgr::~HTTPCookieMgr()
{
  ...
  //Delete the stream source clock
  if (m_pSourceClock)
  {
    QTV_MSG_PRIO(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MED,
                 "~HTTPCookieMgr delete source clock");
    QTV_Delete(m_pSourceClock);
    m_pSourceClock = NULL;
  }
}
                              ...
                              char *CookieValue)
{
   ...
   if(m_pSourceClock == NULL)
   {
     QTV_MSG_PRIO(QTVDIAG_GENERAL, QTVDIAG_PRIO_ERROR,
                     "HTTPCookieMgr::CreateCookie m_pSourceClock is NULL");
     return false;
   }
  ...
  if(result)
  {
   ...
   if ((true == result) && ((char *)0 != cpszMaxAge))
   {
     ...
     else
     {
       ...
       {
         QTV_MSG_PRIO(QTVDIAG_GENERAL, QTVDIAG_PRIO_ERROR,
                     "HTTPCookieMgr::StoreCookie m_pSourceClock is NULL");
         return false;
       }
       double currMSeconds = m_pSourceClock->GetUTCTimeInMsec();
...
)
{
  ...
  {
    QTV_MSG_PRIO(QTVDIAG_GENERAL, QTVDIAG_PRIO_ERROR,
      "HTTPCookieMgr::GetCookies: m_pSourceClock is NULL");
    return false;
  }
  double currMSeconds = m_pSourceClock->GetUTCTimeInMsec();
...
double HTTPCookieMgr::ParseCookieDate(char *pszIn)
{
...

   {
      double availabilityStartTime =
                         StreamSourceTimeUtils::ConvertSysTimeToMSecFromEpoch(j);
