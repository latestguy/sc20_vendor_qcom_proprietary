...

{
  ...
  if(bOk)
  {
    ...
    UTCTime = StreamSourceTimeUtils::ConvertSysTimeToMSecFromEpoch(LocalTime) + UTCoffsetMsecs;
...
/** @brief
  * This method converts the time in MM_Time_DateTime format(year,month,day,hour,min,sec,msecs) to Milliseconds.
  * It gives the total msecs from epoch time "1970-1-1:00:00:00 upto the input time.
  *
  * @param[in] sSysTime - Time in MM_Time_DateTime format
  *
  * @return Time in msecs.
  */
double StreamSourceTimeUtils::ConvertSysTimeToMSecFromEpoch(const MM_Time_DateTime& sSysTime)
{
  MM_Time_DateTime epochTime = {1970,1,0,1,0,0,0,0};
  return (ConvertSysTimeToMSec(sSysTime) - ConvertSysTimeToMSec(epochTime));
}
