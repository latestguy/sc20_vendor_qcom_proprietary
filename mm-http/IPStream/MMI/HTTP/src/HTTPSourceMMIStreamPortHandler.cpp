...
/**
  ...
  * @param[rout] pBuf - Buf pointer
  * @param[in]   nBufSize - Amount of data requested
  * @param[in]   nOffset - Offset from where Parser Reads
  * @param[in]   nWhence - Seek Direction
  * @param[rout] pnRead - Amount of data read into buf
  * @return
  * DS_SUCCESS - *pnRead indicates the amount of data read
  ...
  */
iSourcePort::DataSourceReturnCode HTTPSourceMMIStreamPortHandler::ReadAt
(
  unsigned char* pBuf,
  ssize_t nBufSize,
  const int64 nOffset,
...
{
  QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                "HTTPSourceMMIStreamPortHandler::ReadAt" );
  DataSourceReturnCode result = DS_FAILURE;

  if (m_pHTTPDataBuffer)
  {
    result = m_pHTTPDataBuffer->ReadAt(pBuf, nBufSize, nOffset, nWhence, pnRead);
  }

  if (result != DS_SUCCESS)
  {
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Error: ReadAt failed %d", result );
  }
  return result;
}
