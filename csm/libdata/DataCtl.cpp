...
bool DataCtl::prepare(std::string blackListName) {
	...
	/**check blacklist,  success return 0*/
	std::string iptCmd = "-L " + blackListName;
	res = runIptablesCmd(iptCmd.c_str(), IptIpV4);
	ALOGE("prepare  res: %d", res);
	if (res) {
		/**New blacklist*/
		iptCmd = "-N " + blackListName;
		res = runIptablesCmd(iptCmd.c_str(), IptIpV4);
		if (res) {
			ALOGE("prepare res: %d, create fail exit!", res);
			return false;
		}
	}else{
		/**Delete blacklist from OUTPUT*/
		iptCmd = "-D OUTPUT -j " + blackListName;
		res = runIptablesCmd(iptCmd.c_str(), IptIpV4);
		if (res) {
			ALOGE("prepare res: %d, delete blacklist fail exit!", res);
			return false;
		}
	}

	/**Add blacklist to OUUTPUT*/
	iptCmd = "-A OUTPUT -j " + blackListName;
	res = runIptablesCmd(iptCmd.c_str(), IptIpV4);
	if (res) {
