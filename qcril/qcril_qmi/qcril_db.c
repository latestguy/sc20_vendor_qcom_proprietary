...
#define QCRIL_PREBUILT_DB_NAME "/data/misc/radio/qcril_prebuilt.db"
#define TELEPHONY_DB_NAME   "/data/data/com.android.providers.telephony/databases/telephony.db"
#define UPGRADE_SCRIPTS_PATH   "/system/vendor/qcril_database/upgrade"
#endif
...
#ifndef VENDOR_DB_NAME
#define VENDOR_DB_NAME "/system/vendor/qcril.db"
#endif
...
#define COMMAND_LEN 100
    ...
    // do copy work
    len = snprintf(cmd_buf, COMMAND_LEN, "cp -f %s %s",
            QCRIL_PREBUILT_DB_NAME, QCRIL_DATABASE_NAME);
    if ( ( len < COMMAND_LEN ) && ( len >= 0 ) )
    {
        ret = system(cmd_buf);
        if (WEXITSTATUS(ret))
        {
...
{
    const char presql[] = "PRAGMA main.locking_mode = EXCLUSIVE;BEGIN EXCLUSIVE;COMMIT;"
        "ATTACH DATABASE \"" VENDOR_DB_NAME "\" AS vendordb;";
    const char postsql[] =
    ...
    do {
        ...
        QCRIL_LOG_DEBUG("Error message on Exclusive lock: %s", error);
        if (rc != SQLITE_OK)
        {
            QCRIL_LOG_ERROR("Unable to attach " VENDOR_DB_NAME ": %s", error);
            sqlite3_free(error);
            break;
        }
        ...
        else
        {
            ...
            ** and will retry till qcril_db_busy_handler returns 0.*/
            ...
            if ( qmi_ril_get_process_instance_id() == QCRIL_DEFAULT_INSTANCE_ID )
            {
                ...
                QCRIL_LOG_INFO("Initial value of ADB prop(%s): %d",QCRIL_DB_UPGRADE_COMPLETE,db_upgraded_initial);
                QCRIL_LOG_INFO("Setting value of ADB prop(%s) to: %d",QCRIL_DB_UPGRADE_COMPLETE,0);
                ...
                QCRIL_LOG_INFO("Setting value of ADB prop(%s) to: %d",QCRIL_DB_UPGRADE_COMPLETE,1);
            ...
            else
            {
                ...
                do
                {
                    qmi_ril_get_property_value_from_integer(QCRIL_DB_UPGRADE_COMPLETE,
                                            &db_upgraded, 0);
                    QCRIL_LOG_INFO("Value of ADB prop(%s): %d",QCRIL_DB_UPGRADE_COMPLETE,db_upgraded);
                    if(db_upgraded == 1)
                    {
                        qmi_ril_set_property_value_to_integer(QCRIL_DB_UPGRADE_COMPLETE, 0);
                    ...
                    sleep(1);
                    retry_count++;
                    QCRIL_LOG_INFO("retry count - %d DB Operations done - %d", retry_count, db_upgraded);
