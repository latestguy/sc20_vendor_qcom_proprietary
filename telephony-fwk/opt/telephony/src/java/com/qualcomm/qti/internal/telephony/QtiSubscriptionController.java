...
public class QtiSubscriptionController extends SubscriptionController {
    ...
    protected Bitmap getIconBitmap(Cursor cursor) {
        ...
        if (isCustomSimFeatureEnabled) {
            ...
            String currentMccMncString = "" + mcc + "" + mnc;
            int currentMccMnc = Integer.parseInt(currentMccMncString);
            boolean isSimTypeOperator = operatorSimInfo.isSimTypeOperatorForMccMnc(currentMccMnc);
            if (isSimTypeOperator) {
                 Drawable operatorDrawable = operatorSimInfo.getOperatorDrawable();
                 iconBitmap = ((BitmapDrawable)operatorDrawable).getBitmap();
            } else {
                 Drawable genericSimDrawable = operatorSimInfo.getGenericSimDrawable();
                 iconBitmap = ((BitmapDrawable)genericSimDrawable).getBitmap();
            }
        } else {
            return super.getIconBitmap(cursor);
        }
        return iconBitmap;
    }
    ...
    protected int getIconTint (Cursor cursor) {
        ...
        boolean isCustomSimFeatureEnabled = operatorSimInfo.isOperatorFeatureEnabled();
        if (isCustomSimFeatureEnabled) {
            iconTint = android.R.color.transparent;
        } else {
            return super.getIconTint(cursor);
        }
        return iconTint;
    }
}
