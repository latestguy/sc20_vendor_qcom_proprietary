...
public class QtiPrimaryCardController extends Handler{
    ...
    private static final int MSG_SET_PRIMARY_SUB = 6;
    ...
    private QtiPrimaryCardController(Context context, Phone[] phones,
        ...
        if (SubsidyLockSettingsObserver.isSubsidyLockFeatureEnabled()) {
            new SubsidyLockSettingsObserver(mContext).observe(mCardInfoMgr, mPriorityHandler);
        }
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case EVENT_PRIMARY_CARD_SET_DONE:
                ...
                break;
            case MSG_SET_PRIMARY_SUB:
                logd("on MSG_SET_PRIMARY_SUB ");
    ...
    private void handleSetNwModeDone(Message msg) {
        ...
        if (ar.exception != null) {
            ...
            // When the automatic primary card selection(i.e FWK_PRIMARY_CARD_REQUEST) request fails
            // and if the max re-try count(mRetryCount) is not reached the limit, re-initiate the
            // FWK Primary card request again.
    ...
    private void setDdsOnPrimaryCardIfRequired() {
        ...
        int subId = subCtrlr.getSubIdUsingSlotId(mPrefPrimarySlot)[0];
    ...
    private void handleOnSetPrimaryCardDone(Message msg) {
        ...
        if (enableUserSelection && mCardChanged && numCardsValid() > 1
                && isPermanentlyUnlocked && mPrimaryCardState == PrimaryCardState.IDLE) {
    ...
    public void trySetPrimarySub(boolean force) {
        sendMessage(obtainMessage(MSG_SET_PRIMARY_SUB));
    }
    ...
    */
    ...
    synchronized private void setPrimaryCardOnSlot(int primarySlotId, Message msg,
        ...
        if ((mPrimaryCardState != PrimaryCardState.IDLE && !isRetryRequest)
                ...
                || !SubscriptionManager.isValidSlotId(primarySlotId)
                || !subCtrlr.isActiveSubId(subId)) {
        ...
        mCmdMessage = msg;
        int isFwkRequest =
                (mCmdMessage != null) ? FWK_PRIMARY_CARD_REQUEST : USER_PRIMARY_CARD_REQUEST;
        mPcTriggeredFlexMapDone = false;
