...
public:
  ...
  static void sendUnsolicitedMsg(int targetFd, cne_msg_enum_type msgType, T const& data) {
    ...
    uint8_t buffer[CNE_MAX_VENDOR_DATA_LEN];
    void *const p = &buffer;
    int bodySz = CneParcel::parcel(data, p);
    sendUnsolicitedMsg(targetFd, msgType, bodySz, p);
  }

  /*
   * sendUnsolicitedMsg of msgType to targetFd with notify flag.
   */
  template <class T>
  static void sendUnsolicitedMsg(int targetFd, cne_msg_enum_type msgType,
        T const& data, bool notify){
    CNE_MSG_VERBOSE("sending unsolicited message. fd:%d type:%s (%d), notify: %s",
        targetFd, CneUtils::getCneMsgStr(msgType), msgType, notify?"true":"false");
    uint8_t buffer[CNE_MAX_VENDOR_DATA_LEN];
    void *const p = &buffer;
    int bodySz = CneParcel::parcel(data, notify, p);
    sendUnsolicitedMsg(targetFd, msgType, bodySz, p);
  }
  /*----------------------------------------------------------------------------
   * FUNCTION      sendUnsolicitedMsg
