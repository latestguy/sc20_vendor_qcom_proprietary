...
int32_t get_voc_afe_port_id(ACPH_CMD_QUERY_VOC_AFE_ACTIVE_PORT_IDS_V2_req *voc_afe_info,
			uint32_t *voc_afe,
			uint32_t *bytes_filled)
{
	int ret = 0;
	...
	voc_afe[3] = rtac_voice_data.voice[i].rx_afe_port;
				...
				uint32_t resp_buf_len, uint32_t *resp_buf_bytes_filled)
{
	...
	case ACPH_CMD_QUERY_VOC_AFE_ACTIVE_PORT_IDS_V2:
		ret = get_voc_afe_port_id((ACPH_CMD_QUERY_VOC_AFE_ACTIVE_PORT_IDS_V2_req *)req_buf_ptr,
					(uint32_t *)resp_buf_ptr,
					resp_buf_bytes_filled);
		goto done;
