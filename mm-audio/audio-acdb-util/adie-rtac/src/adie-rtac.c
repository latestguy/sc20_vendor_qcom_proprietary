...
static int find_codecs_info(void)
{
	...
	while ((dentry = readdir(dir)) != NULL) {
		...
		if( ret < 0 && !strncmp(codec_info[number_of_codecs].codec_name,"tasha",
						strlen(codec_info[number_of_codecs].codec_name)+1))
		{
			strlcpy(codec_info[number_of_codecs].codec_name,"-000d",
