...
typedef struct vbat_monitor_reg_t {
    ...
    // 16 bits of write[x] applicable for api convert_anc_acdb_to_reg(...) & convert_anc_reg_to_acdb(...) is partitioned as [reg (msb 16 bits), mask(8 bits) , val (8 bits)]
    // 16 bits of write[x] applicable for api get_anc_reg_read_sequence(...) is partitioned as [reg (msb 16 bits), direction(8 bits), val (8 bits)]
} vbat_monitor_reg_t;
