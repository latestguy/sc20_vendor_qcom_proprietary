...
SoftFlacDecoder::~SoftFlacDecoder() {
    ...
    if (mDecoderDeInit && mInitStatus) {
        (*mDecoderDeInit) (&flacDecState);
        mDecoderDeInit = NULL;
    }
...
OMX_ERRORTYPE SoftFlacDecoder::init(FLACDec_ParserInfo* parserInfoToPass) {
    ...
    mDecoderDeInit = (DecoderDeInit) dlsym (mLibHandle, "CFlacDecoderLib_End");

    if (!mDecoderInit || !mProcessData || !mDecoderDeInit) {
