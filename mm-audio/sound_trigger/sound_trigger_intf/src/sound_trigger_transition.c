...
#include "sound_trigger_prop_hw.h"
...
static int transit_route_set_device(sound_trigger_session_t* p_ses,
                                bool enable);
static int transit_route_pre_dev_switch(sound_trigger_session_t* p_ses);
static int transit_route_post_dev_switch(sound_trigger_session_t* p_ses);
                                ...
                                bool enable)
{
   ...
   int idx = 0, status = 0;
    ...
    if (enable)
        device = platform_stdev_get_capture_device(stdev->platform);
    else
        device = stdev->capture_device;
...
static int transit_route_pre_dev_switch(sound_trigger_session_t* p_ses)
{
    ...
    if (CHECK_STATE(p_ses->state, SES_TRANSIT_STARTED))
        status = cpe_pre_dev_switch(p_ses->transit_ses);
    else
        status = ape_pre_dev_switch(p_ses);
...
static int transit_route_post_dev_switch(sound_trigger_session_t* p_ses)
{
    ...
    if (CHECK_STATE(p_ses->state, SES_TRANSIT_STARTED))
        status = cpe_post_dev_switch(p_ses->transit_ses);
    else
        status = ape_post_dev_switch(p_ses);
