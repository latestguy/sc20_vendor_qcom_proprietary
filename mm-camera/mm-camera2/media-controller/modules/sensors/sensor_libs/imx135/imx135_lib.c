...
static struct sensor_lib_chromatix_t imx135_chromatix[] = {
#ifdef _FULL_RES_30FPS
  {
    ...
    .adrc_chromatix = IMX135_LOAD_CHROMATIX(adrc),
 ...
 *==========================================================================*/
static int32_t parse_VHDR_stats(uint32_t *destLumaBuff, void *rawBuff)
{
  uint8_t       *buf = NULL;
  uint8_t       stat_seq[10];
  uint32_t      temp_y = 0;
  ...
  for (i = 0; i < 16; i++) {
    for (j = 0; j < 16; j++) {
      ...
      SLOW("%s: Before moving. buf: %p", __func__, buf);
      buf = buf + 10;
      SLOW("%s: After moving. buf: %p", __func__, buf);
      ...
      temp_y = temp_y << 6;
      temp_y |= (stat_seq[1] >> 2);
      SLOW("%s: Value obtained for block (%d, %d): %d", __func__,
        i, j, temp_y);
...
static sensor_parse_RDI_stats parse_RDI_stats = {
...
static sensor_lib_t sensor_lib_ptr = {
  ...
  .parse_RDI_stats = &parse_RDI_stats,
};
