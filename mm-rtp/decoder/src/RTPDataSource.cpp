...
namespace android
{
  ...
  ssize_t RTPDataSource::readAt(int64 offset, void *data, size_t size)
  {
    ...
    if(!data || !size)
    {
      MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "RTPDataSource:readAt invalid args");
      return 0;
    }
    if(m_eRTPState == RTP_FETCHING_DATA)
    {
...
#endif /*RTP_READ_FROM_FILE*/
      {
        ...
        /*Don't call RTPParser read, if complete requested data is not available as RTPParser read is blocking call
              */
        if((int64)(offset + TS_PKT_LEN) <= numBytesAvailable)
        {
          /* Read only available downloaded data*/
          ssize_t nReadSize = numBytesAvailable - offset;
          if(nReadSize > size)
          {
            nReadSize = size;
          }
          nBytes = m_pRTPParser->read(offset, data, nReadSize);
        }
        else
        {
          MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_MEDIUM, "RTPDataSource:readAt data is not available,"
  ...
  int32 RTPDataSource::fetchData()
  {
    ...
    if( nBytes > 0)
    {
      int validData = (nBytes - RTP_PACKET_HEADER_LENGTH);
      if(validData > 0)
      {
        int tsPktNum = validData / TS_PKT_LEN;
        int remSize = validData % TS_PKT_LEN;
        validData = 0;
        if((tsPktNum > 0 && tsPktNum <= 7) && (remSize == 0))
        {
            validData = 1;
        }
        else
          MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "TCP : ERROR : Size of packet for parsing = %ld",nBytes);
      }
      else
      {
        validData = 0;
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "TCP : ERROR s12 : Size of packet for parsing = %ld",nBytes);
      }
      if(validData)
    ...
    {
      /* No data available in this case, as the system call to receive data from socket was interrupted
         by a signal. There is a good chance of recovery from here by attempting a retry */
      MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "RTPDataSource:Interrupted system call, attempt retry");
    }
    else
    {
  ...
  int32 RTPDataSource::setSocketOptions(int32 socket)
  {
    ...
    if( recvBufferSize < (RTP_SOCKET_RECEIVE_BUFFER_SIZE * 2) )
    {
      ...
      if( 0 != setsockopt((int)socket, SOL_SOCKET, SO_LINGER, &lingerInfo, sizeof(lingerInfo)))
      {
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTPDataSource:Error %d in setting socket SO_LINGER", errno);
        return nStatus;
      }
    }
