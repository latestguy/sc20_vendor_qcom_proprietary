...
/*Static member definitions*/
   ...
   ssize_t RTPParser::read(int64 offset, void *data, size_t size)
   {
      ...
      int nBytes = 0;
      int nLoopCtr = 2; //Twice when data wraps around buffersize
      while(nLoopCtr--)
      {
         ...
         size -= chunk;
         nBytes += chunk;
         if(chunk == availableLength)
         {
           break;
         }
         else
         {
           MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "RTPParser::Read wraps around");
         }
      }
   ...
   /*   if(m_bIsTCP)
    ...
    void RTPParser::updateRTPPortVars()
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_DEBUG,
            "RTPParser:Reset required member variables");
        mFirstPacketBaseTimeUs = -1;
