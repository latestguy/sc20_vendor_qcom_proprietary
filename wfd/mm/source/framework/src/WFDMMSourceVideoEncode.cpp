              ...
              void* pAppData)
{
    ...
    int nLowLatencyMode = 0; //Disable low latency mode by default
    int nPowerSaveMode = 1; //Enable max quality by default
    int nPriority = 0; //Enable real time mode by default
    ...
    {
       inPortDef.nBufferCountActual = m_nNumInputBuffers;
    }
    else
    {
       m_nNumInputBuffers = inPortDef.nBufferCountActual =
                               inPortDef.nBufferCountMin;
    }
    result = OMX_SetParameter(m_hEncoder,
                              OMX_IndexParamPortDefinition,
                              (OMX_PTR) &inPortDef);
...
// TODO: Does not compile as it is
    ...
    */
    ...
    /*--------------------------------------------------------------------------
      Enabling low_latency mode to configure encoder in 1D1S mode
    ----------------------------------------------------------------------------
    */
    nRetVal = getCfgItem(LOW_LATENCY_MODE_KEY, &nLowLatencyMode);

    if(nRetVal == 0)
    {
        QOMX_ENABLETYPE low_latency;
        low_latency.bEnable = static_cast<OMX_BOOL>(nLowLatencyMode);
        ...
        WFD_OMX_ERROR_CHECK(result,
            "Cannot set OMX_QcomIndexConfigVideoVencLowLatencyMode");
        WFDMMLOGH1("Setting Low Latency Mode with value [%d]",nLowLatencyMode);
    }

    /*--------------------------------------------------------------------------
      Setting power save mode for low quality encoding
    ----------------------------------------------------------------------------
    */
    nRetVal = getCfgItem(ENCODER_POWER_SAVE_MODE_KEY, &nPowerSaveMode);

    if(nRetVal == 0)
    {
        QOMX_EXTNINDEX_VIDEO_PERFMODE perf_mode;
        OMX_INIT_STRUCT(&perf_mode,QOMX_EXTNINDEX_VIDEO_PERFMODE);
        perf_mode.nPerfMode = nPowerSaveMode; /*1: MAX_QUALITY 2: POWER_SAVE*/
        ...
        WFD_OMX_ERROR_CHECK(result,
            "Cannot set OMX_QcomIndexConfigVideoVencPerfMode");
        WFDMMLOGH1("Setting power save mode with value [%d]", nPowerSaveMode);
    }

    /*--------------------------------------------------------------------------
      Setting Real time mode for encoder
    ----------------------------------------------------------------------------
    */
    nRetVal = getCfgItem(ENCODER_REAL_TIME_MODE_KEY, &nPriority);

    if(nRetVal == 0)
    {
        OMX_PARAM_U32TYPE enc_priority;
        OMX_INIT_STRUCT(&enc_priority, OMX_PARAM_U32TYPE);
        enc_priority.nU32 = nPriority; /* 0: Real Time else Non-Real Time */

        result = OMX_SetConfig(m_hEncoder,
                     (OMX_INDEXTYPE)OMX_IndexConfigPriority,
                     (OMX_PTR)&enc_priority);

        WFD_OMX_ERROR_CHECK(result, "Cannot set OMX_IndexConfigPriority");
        WFDMMLOGH1("Setting Priority = %d for encoder", nPriority);
    }
...
OMX_ERRORTYPE WFDMMSourceVideoEncode::createResources()
{
    ...
    m_pInpBufQ = MM_New_Args(SignalQueue,(100, sizeof(OMX_BUFFERHEADERTYPE*)));
    m_pOutBufQ = MM_New_Args(SignalQueue,(100, sizeof(OMX_BUFFERHEADERTYPE*)));
      ...
      OMX_IN OMX_PTR pEventData)
{
    ...
    if (eEvent == OMX_EventCmdComplete)
    {
        if ((OMX_COMMANDTYPE) nData1 == OMX_CommandStateSet)
        {
            ...
            WFDMMLOGE2("Pushed cmd with %d and %d", cmd.eEvent,cmd.eCmd);
        }
...
OMX_ERRORTYPE WFDMMSourceVideoEncode::Pause()
{
    ...
    */
    ...
    {
        int nRetryIteration = 50;
        result = OMX_ErrorUndefined;

        while (result != OMX_ErrorNone && nRetryIteration--)
        {
            result = m_pCommandQ->Pop(&cmd, sizeof(cmd), 100);
            if (result != OMX_ErrorTimeout && result != OMX_ErrorNone)
            {
                WFD_OMX_ERROR_CHECK(result,"Error in popping from Signal Queue");
            }
        }
        WFD_OMX_ERROR_CHECK(result,"Error in popping command");
