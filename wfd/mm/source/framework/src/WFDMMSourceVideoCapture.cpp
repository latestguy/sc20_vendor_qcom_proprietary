...
OMX_ERRORTYPE WFDMMSourceVideoCapture::createSurface(int width, int height)
{
    ...
    m_pSurfaceMediaSrc   = MM_New_Args(WFDSurfaceMediaSource,
                                       (width, height,WFD_SMS_CONSUMER_HW));
...
OMX_ERRORTYPE WFDMMSourceVideoCapture::stopCapture()
{
    ...
    if(m_hVideoCaptureTimer)
    {
        ...
        WFDMMLOGD("CaptureTimer stopped");
    }
    ...
    while(m_pVideoInputQ && m_pVideoInputQ->GetSize())
    {
        ...
        */
        ...
        WFDMMLOGD1("stopCapture:: Queue Size is %ld",
                m_pVideoInputQ->GetSize());
        m_pVideoInputQ->Pop(&pBufferHdr, sizeof(&pBufferHdr), 0);
        if(pBufferHdr)
        {
            WFDMMLOGD1("stopCapture:: popped %p", pBufferHdr);
    ...
    WFDMMLOGD("stopCapture:: releaseMediaBuffers");
    releaseMediaBuffers();
...
OMX_ERRORTYPE WFDMMSourceVideoCapture::pauseCapture()
{
    ...
    /*--------------------------------------------------------------------------
     Wait to avoid race condition in releasing MediaBuffers
    ----------------------------------------------------------------------------
    */
    long stime = 1000/m_sConfig.nFramerate;
    WFDMMLOGH2("Sleeping %ld ms (FPS %u) to ensure sanity in releaseMediaBuffers",
        stime, m_sConfig.nFramerate);
    MM_Timer_Sleep(stime);
...
void WFDMMSourceVideoCapture::inputDeliveryHandler()
{
    ...
    */
    ...
    else if(m_sConfig.bEnableFrameSkip)
    {
        ...
        if(nFramesRead == 1 && (m_nTimerCtr % 2 == 0))
        {
            /*------------------------------------------------------------------
             No frame drop case.
            --------------------------------------------------------------------
            */
            WFDMMLOGH("FRC: Frame Skip enabled:send new frame");
            ...
            if(pBufferHdr[0]->pBuffer && pBufferHdr[0]->pOutputPortPrivate)
            {
                ...
                if((char*)pMediaBuffer->data())
                {
                    ...
                    pBufferHdr[0]->nFilledLen = (OMX_U32)pMediaBuffer->size();//range_length();
        ...
        {
            if((((((OMX_U32)nCurrTime - (OMX_U32)m_nPrevBufferTS))
                    > m_sConfig.nFrameSkipInterval*1000)
                    || m_nMinRetransmitCnt) && (m_nTimerCtr % 2 == 0))
            {
                ...
                */
                ...
                if(pBufferHdr[0]->pBuffer && pBufferHdr[0]->pOutputPortPrivate)
                {
                    ...
                    if((char*)pMediaBuffer->data())
                    {
                        ...
                        pBufferHdr[0]->nFilledLen = (OMX_U32)pMediaBuffer->size();
                        fillStatInfo(pBufferHdr[0],nCurrTime,pMediaBuffer);
                    }
                }
                m_pFnData(m_pAppData, pBufferHdr[0]);
        ...
        if(pBufferHdr[1])
        {
            ...
            if(pBufferHdr[1]->pBuffer && pBufferHdr[1]->pOutputPortPrivate)
            {
                ...
                if((char*)pMediaBuffer->data())
                {
                    ...
                    pBufferHdr[1]->nFilledLen =  (OMX_U32)pMediaBuffer->size();//range_length();
