...
*//*==========================================================================*/
  OMX_ERRORTYPE AudioSource::SetFreeBuffer(OMX_BUFFERHEADERTYPE* pBufferHdr)
  {
      ...
      if (pBufferHdr != NULL && pBufferHdr->pBuffer != NULL)
      {
          ...
          m_numFramesReceivedBack++;
          MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM,
             "SetBuffer: Passing audio buffer from MUXER DownStreamBuf = %u",
                                            (m_numFramesReceivedBack));
      ...
      if (pThis->bKeepGoing == OMX_TRUE )
      {
         ...
         if(OMX_FALSE == pThis->m_bIsProxyOpened)
         {
           ...
           if(OMX_TRUE == bInsertSilence)
           {
             ...
             pThis->AudioSource_sleep_for_audioframe_interval((unsigned long)nTimeTaken_ns);
           }
         }
         else
         {
...
// 2. Append Private Data as Extra Data to the end of Elementary Stream Buffer
          ...
          pThis->m_pHdcpHandle->m_eHdcpSessionStatus == HDCP_STATUS_SUCCESS )
      {
        ...
        // H/w lib
        signed long ulStatus = pThis->m_pHdcpHandle->WFD_HdcpDataEncrypt(STREAM_AUDIO ,
              ...
              if (pThis->pBufferHdr->nFilledLen)
              {
                     ...
                     (OMX_U64)nTimeStamp - (OMX_U64)pThis->m_nCurrTime > nDriftCutoff)
                  {
                     ...
                     if(pThis->m_nTimeErrAdjustCnt && pThis->m_nFrameTimeUnit)
                     {
                         ...
                         pThis->m_nTimeErrAdjustCntr = (OMX_U32)
                  ...
                  pThis->m_numFramesSent++;
                  MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM,
                              "AudioSource:: sent buffers= %u, received buffers = %u ",
                              pThis->m_numFramesSent,pThis->m_numFramesReceivedBack);
              ...
              case AUDIO_SOURCE_PLAY_EVENT:
                  {
                      ...
                      //initialize sent buffers with number of buffers
                      m_numFramesSent = m_nBuffers;
                      m_numFramesReceivedBack = 0;
                      for (int i = 0; i < m_nBuffers; i++)
                      {
              ...
              case AUDIO_SOURCE_PAUSE_EVENT:
                  {
                      ...
                      // compare sent buffers count with received buffers count to
                      // make sure all buffers are returned back from mux
                      while(m_numFramesSent != m_numFramesReceivedBack)
                      {
                        MM_Timer_Sleep(1);
                        MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
                              "AudioSource:: sent buffers= %u, received buffers = %u ",
                              m_numFramesSent,m_numFramesReceivedBack);
                      }
