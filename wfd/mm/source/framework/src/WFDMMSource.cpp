...
OMX_ERRORTYPE WFDMMSource::InitializeThreadsAndQueues()
{
    ...
    */
    ...
    /**---------------------------------------------------------------------------
    Create the linkspeed timer if DBRA is enabled.
    ------------------------------------------------------------------------------
    */
    int nEnabled = 0;
    if(getCfgItem(DYN_BIT_ADAP_KEY, &nEnabled) == 0 && nEnabled)
    {
        if(!m_hLnkSpdTmr)
        {
            if(0 != MM_Timer_CreateEx(1,WFDMMSource::LinkSpeedCheckTimerCb,
  ...
  if( NULL!= m_pNegotiatedCap)
  {
      if(m_pNegotiatedCap->HdcpCtx.hHDCPSession)
      {
          ...
          )
        {
            ...
            {
                MM_Timer_Sleep(100);
                MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_LOW, "WFD MMSource HDCP status %d at count= %d",
                             m_pNegotiatedCap->HdcpCtx.hHDCPSession->m_eHdcpSessionStatus,count);
            }
            if(m_pNegotiatedCap->HdcpCtx.hHDCPSession->m_eHdcpSessionStatus !=
                HDCP_STATUS_SUCCESS)
            {
...
WFDMMSource::~WFDMMSource()
{
    ...
    if(m_hLnkSpdTmr)
    {
        int res = MM_Timer_Stop(m_hLnkSpdTmr);
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
              "WFDMMSource::Stopping Linkspeed timer %d",res);
        res = MM_Timer_Release(m_hLnkSpdTmr);
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
...
OMX_ERRORTYPE WFDMMSource::Play()
{
    ...
    if(m_hLnkSpdTmr)
    {
        if(0 != MM_Timer_Start(m_hLnkSpdTmr,MM_LINKSPEED_CHECK_INTERVAL))
        {
          MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                                  "WFDMMSource:: Linkspeed Timer failed");
        }
    ...
    switch (eCommand)
    {
      ...
      case WFD_MM_CMD_ENABLE_BITRATE_ADAPT:
      {
        if (m_eState == MMWFDSRC_STATE_PLAY)
        {
          ...
          {
              /**---------------------------------------------------------------------------
              DBRA was enabled already and timer is created, just start the
              timer.
              ------------------------------------------------------------------------------
              */
            if(0 != MM_Timer_Start(m_hLnkSpdTmr,MM_LINKSPEED_CHECK_INTERVAL))
      ...
      case WFD_MM_CMD_DISABLE_BITRATE_ADAPT:
      {
        ...
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                       "WFDMMSource:: Linkspeed timer not created");
            return OMX_ErrorInsufficientResources;
        }
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
      ...
      case WFD_MM_CMD_HANDLE_STANDBY:
      {
#ifdef WFD_VDS_ARCH
        if(m_pNegotiatedCap->standby_resume_support)
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
              "WFDMMSource:: WFD_MM_CMD_HANDLE_STANDBY");
            retval = m_pVideoSource->HandleStandby();
        }
        else
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
              "WFDMMSource:: HANDLE_STANDBY ignored, no standby support");
                                           ...
                                           uint32 hh_mode )
{
    ...
    if(cea_mode)
    {
...

        {
            ...
            if ((OMX_U32)tempMaxBitrate <= m_nMaxBitrate)
            {
