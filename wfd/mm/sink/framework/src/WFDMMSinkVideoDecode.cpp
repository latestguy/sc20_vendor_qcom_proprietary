...
/*Input buffer size need to be aligned 4KB(for non secure)
  and 1 MB aligned for Secure. Value is for 1080p Max
*/
#define MAX_INPUT_BUFFER_SIZE (3*1024*1024)
...
//                sPortDef.format.video.nStride      = mCfg.nFrameWidth;
    ...
    */
    ...
    /*--------------------------------------------------------------------------
     Set Custom Buffer Size for proper memory allocation
    ----------------------------------------------------------------------------
    */
    ...
    sCustomBufferSize.nBufferSize = MAX_INPUT_BUFFER_SIZE; // 3MB
    ...
    if (eRet != OMX_ErrorNone)
    {
        WFDMMLOGE("Failed to set Custom Buffer Size");
    }
