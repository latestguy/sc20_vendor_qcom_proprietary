...
OMX_ERRORTYPE WFDMMSinkAACDecode::createResources()
{
    ...
    /*--------------------------------------------------------
       Making sure that default configuration is set properly.
       In case, max output channel is less than input channels
       then down mixing happens and in case of channel > 6, there
       are decoding errors.
       For such cases, we can configure it to -1 or 0 to disable
       downmixing feature. The decoder output contains the
       same number of channels as the encoded bitstream.
      -------------------------------------------------------
    */
    int nChannels = mCfg.nChannels;
    if(mCfg.nChannels > 6)
    {
        nChannels = -1;
    }
    int ret = aacDecoder_SetParam((HANDLE_AACDECODER)mhAACDec,
                        AAC_PCM_MAX_OUTPUT_CHANNELS,
                        nChannels);

    WFDMMLOGH2("AudioDecoder Initialized: channel = %d , channel set param = %d"
              ,nChannels,ret);
...
OMX_ERRORTYPE WFDMMSinkAACDecode::Decode(OMX_BUFFERHEADERTYPE *pBuffer)
{
    ...
    */
    ...
    /*-----------------------------------------------------------------------------
      Our input buffer is allocated of type CHAR where as we are typecasting it to
      INT_PCM while sending to decode. INT_PCM is of type SHORT which is twice CHAR.
      So, this makes alloc length as a wrong data because we didn't allocate those
      many elements in array.

      As aacDecoder_DecodeFrame() expects INT_PCM type only, hence correcting
      alloc length accordingly to reflect correct number of data.
     ------------------------------------------------------------------------------
     */
    int nConFactor = sizeof(INT_PCM)/sizeof(UCHAR);
    int nAllocLen = (nConFactor == 0) ? pBuffer->nAllocLen
                                      : pBuffer->nAllocLen/nConFactor;
