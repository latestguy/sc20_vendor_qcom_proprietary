...
WFDMMSink::WFDMMSink()
{
    ...
    m_bMissedAudioStartTimeFlag = false;
    m_bMissedVideoStartTimeFlag = false;
    m_bMissedRendererBaseTimeSet = false;
    m_nBaseTime = 0;
                                    ...
                                    wfd_mm_update_session_cb pUpdateSessionCb)
{
    ...
    WFDMMLOGH2("WFDMMSink:: UpdateSession: eRtpPortType[%d] port1_id[%d]",
            m_pNegotiatedCap->transport_capability_config.eRtpPortType,
            m_pNegotiatedCap->transport_capability_config.port1_id);
                           ...
                           OMX_BUFFERHEADERTYPE *pBuffer)
{
    ...
    case SINK_MODULE_MEDIASOURCE:
        if(!IS_MM_STATE_PLAY)
        {
            WFDMMLOGE1("Not in PLAY state give buffer back to MediaSrc. TRACK[%d]",trackID);

            /*------------------------------------------------------------------
             In the event OMX_BUFFERFLAG_STARTTIME is set by MediaSource but the
             buffer is not being delivered further to intended components, allow
             WFDMMSink to keep hold of this information so as to use it at the
             required time.
            --------------------------------------------------------------------
            */
            getSetBufferStartTimeFlag(trackID, pBuffer,true);
        ...
        /*----------------------------------------------------------------------
         In the event Renderer base time setting has been missed due to delay in
         its construction (TCP usecase where MediaSrc kicks off early), set the
         renderer base time once to prevent renderer going for a toss
        ------------------------------------------------------------------------
        */
        if(m_bMissedRendererBaseTimeSet && m_pRenderer)
        {
            WFDMMLOGH1("WFDMMSink:: Setting missed renderer base time now [%llu]",
                m_nBaseTime);
            m_pRenderer->SetMediaBaseTime(m_nBaseTime);
            m_bMissedRendererBaseTimeSet = false;
        }
...
#endif
            {
                getSetBufferStartTimeFlag(trackID, pBuffer, false);
                if(!m_pRenderer ||
...
void WFDMMSink::setAvInfo(avInfoType *pInfo)
{
    if(pInfo)
    {
        ...
        {
            WFDMMLOGH("WFDMMSink Bookeeping missed Setting Renderer baseTime flag");
            m_bMissedRendererBaseTimeSet = true;
        }
    }
}
...
/*==============================================================================
         ...
         FUNCTION:         getSetBufferStartTimeFlag

         DESCRIPTION:      Checks for OMX_BUFFERFLAG_STARTTIME flag in the buffer
*//**       @brief         or sets the same in the buffer
*//**
@par     DEPENDENCIES:
                            None
*//*
         PARAMETERS:
*//**       @param          int - Track ID to distinguish between AV buffers
                            OMX_BUFFERHEADERTYPE* - AV OMX_BUFFERHEADERTYPE
                            bool - gets or sets the OMX_BUFFERFLAG_STARTTIME
...
@par     SIDE EFFECTS:
                            None
*//*==========================================================================*/
void WFDMMSink::getSetBufferStartTimeFlag(int trackID,
                                          OMX_BUFFERHEADERTYPE * pBuffer,
                                          bool get)
{
    if(!pBuffer)
    {
        WFDMMLOGE("WFDMMSink:: pBuffer received is NULL");
        return;
    }
    ...
    if(get)
    {
        if((pBuffer->nFlags & OMX_BUFFERFLAG_STARTTIME))
        {
            switch(trackID)
            {
                ...
                {
                    WFDMMLOGH("WFDMMSink::Bookkeeping STARTTIME Flag for Video");
                    m_bMissedVideoStartTimeFlag = true;
                }
                break;

                case SINK_AUDIO_TRACK_ID:
                {
                    WFDMMLOGH("WFDMMSink:: Bookeeping STARTTIME Flag for Audio");
                    m_bMissedAudioStartTimeFlag = true;
                }
                break;

                default:
                    WFDMMLOGE("WFDMMSink:: Incorrect TrackID received");
                break;
            }
        }
    }
    else
    {
        switch(trackID)
        {
            case SINK_VIDEO_TRACK_ID:
            {
                if(m_bMissedVideoStartTimeFlag)
                {
                    WFDMMLOGH("WFDMMSink:: Setting missed STARTTIME for Video");
                    pBuffer->nFlags |= OMX_BUFFERFLAG_STARTTIME;
                    m_bMissedVideoStartTimeFlag = false;
                }
            }
            ...
            case SINK_AUDIO_TRACK_ID:
            {
                if(m_bMissedAudioStartTimeFlag)
                {
                    WFDMMLOGH("WFDMMSink:: Setting missed STARTTIME for Audio");
                    pBuffer->nFlags |= OMX_BUFFERFLAG_STARTTIME;
                    m_bMissedAudioStartTimeFlag = false;
                }
            }
