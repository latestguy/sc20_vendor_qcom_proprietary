...
)
{
    ...
    */
    ...
    {
        strlcpy(mName,"WfdMMThread",sizeof(mName));
    }

    mStartForeground = startForeground;
...
{
    bool bRunning = true;
    WFDMMLOGH1("%s:: loop", mName);
    setThreadName(mName);
    WFDMMLOGE3("%s Has scheduling policy %s with timer slack of %lu ns",
        mName, getSchedPolicyName(),getTimerSlackValue());
