    ...
    if(tagvalue.length())
    {
        ...
        if((value == 0) &&
           ((mnHW_ID == 206)|| (mnHW_ID>=239 && mnHW_ID <= 243)||
            (mnHW_ID>=268 && mnHW_ID <= 271)||(mnHW_ID == 264)))
        {
          tagvalue = string("1");
...
//Parse for configurable items here itself in order to cache for later use
   ...
   /* LOW_LATENCY_MODE */
   ret = property_get("debug.wfd.low_latency_mode", szTemp, NULL);
   if(ret <= 0)
   {
     /* Failed to read the property. Set default for selective target(s) */
     switch(mnHW_ID)
     {
       case 266:
       case 274:
       case 277:
       case 278: /* For target where 1D1S mode is required */
       {
         MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
           "Setting LOW_LATENCY_MODE with default = 1");
         tagValue = string("1");
         updateVar(LOW_LATENCY_MODE_KEY, LOW_LATENCY_MODE, tagValue);
         break;
       }
       default:
         break;
   ...
   else
   {
     ...
     MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
       "Overriding LOW_LATENCY_MODE by = %s", szTemp);
     tagValue = string(szTemp);
     updateVar(LOW_LATENCY_MODE_KEY, LOW_LATENCY_MODE, tagValue);
   }

   /* ENCODER_POWER_SAVE_MODE */
   ret = property_get("debug.wfd.enc_power_save_mode", szTemp, NULL);
   if(ret <= 0)
   {
     /* Failed to read the property. Set default for selective target(s) */
     switch(mnHW_ID)
     {
       ...
       case 274:
       case 277:
       case 278: /* For target where low quality encoding is required */
       {
         MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
           "Setting ENCODER_POWER_SAVE_MODE with default = 2");
         tagValue = string("2"); /*1: MAX_QUALITY 2: POWER_SAVE*/

         updateVar(ENCODER_POWER_SAVE_MODE_KEY, ENCODER_POWER_SAVE_MODE, tagValue);
         break;
       }
       default:
         break;
   ...
   else
   {
     /* Exercise caution */
     MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
       "Overriding ENCODER_POWER_SAVE_MODE by = %s", szTemp);
     tagValue = string(szTemp);
     updateVar(ENCODER_POWER_SAVE_MODE_KEY, ENCODER_POWER_SAVE_MODE, tagValue);
   }

   /* ENCODER_REAL_TIME_MODE */
   ret = property_get("debug.wfd.enc_real_time_mode", szTemp, NULL);
   if(ret <= 0)
   {
     /* Failed to read the property. Set default for selective target(s) */
     switch(mnHW_ID)
     {
       ...
       case 274:
       case 277:
       case 278: /* For target where real time mode setting is required */
       {
         MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
           "Setting ENCODER_REAL_TIME_MODE with default value = 0");
         tagValue = string("0"); /*0: REAL TIME */

         updateVar(ENCODER_REAL_TIME_MODE_KEY, ENCODER_REAL_TIME_MODE, tagValue);
         break;
       }
       default:
         break;
   ...
   else
   {
     /* Exercise caution */
     MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
       "Overriding ENCODER_REAL_TIME_MODE by = %s", szTemp);
     tagValue = string(szTemp);
     updateVar(ENCODER_REAL_TIME_MODE_KEY, ENCODER_REAL_TIME_MODE, tagValue);
   }
}
