...
mHIDType(-1)
{
    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"UIBCHIDInjector ctor()");
    if(setup_HID()) {
...
UIBCHIDInjector::~UIBCHIDInjector()
{
    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"UIBCHIDInjector dtor()");
...
{
    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"setup_HID()");
    if(m_eStatus == HID_INVALID_STATE) {
        if(mFd != -1) {
       ...
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH, "setup_HID() successful");
    } else {
...
{
    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"create_uhid_device()");
    ...
    if(m_eStatus == HID_INITIALIZED) {
       ...
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"HID Device creation successful");
    } else {
...
{
    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"In HID_poll_thread");
    ...
    while(m_eStatus != HID_DEINITIALIZED){
        ...
        if (pfds[0].revents & POLLIN) {
            MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH, "Received event from uhid device");
            ret = read_uhid_event();
            if (ret){
                break;
            }
    ...
    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"Exiting HID_poll_thread");
}
...
int UIBCHIDInjector::read_uhid_event()
{
    ...
    switch (ev.type) {
    case UHID_START:
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UHID_START from uhid-dev");
        m_eStatus = HID_DEV_CREATED;
        break;
    case UHID_STOP:
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UHID_STOP from uhid-dev");
            m_eStatus = HID_DEV_DESTOYED;
        break;
        //ToDo: May need to handle these uhid events in the future
    case UHID_OPEN:
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UHID_OPEN from uhid-dev");
        break;
    case UHID_CLOSE:
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UHID_CLOSE from uhid-dev");
        break;
    case UHID_OUTPUT:
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UHID_OUTPUT from uhid-dev");
        break;
    case UHID_OUTPUT_EV:
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UHID_OUTPUT_EV from uhid-dev");
        break;
    case UHID_FEATURE:
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UHID_FEATURE from uhid-dev");
        break;
    default:
...
int UIBCHIDInjector::send_report(uint8* report, size_t len)
{
    ...
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"Sending report to uhid device");
    struct uhid_event ev;
    memset(&ev, 0, sizeof(ev));
    ev.type = UHID_INPUT;
...
{
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"%s",__FUNCTION__);
    if(NULL!= mRepDesc) {
        delete mRepDesc;
    }
        ...
        if(rDesc) {
           ...
           memcpy(mRepDesc->repDesc,rDesc,len);
            ...
            if(mHIDType == HID_KEYBOARD) {
               MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"Setting rdesc for Keyboard");
               ...
               MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"Setting rdesc for Mouse");
...
{
    if(hidType != mHIDType) {
      MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "set_HID_type called");
      ...
      if(m_eStatus == HID_INVALID_STATE) {
        ...
        mHIDType = hidType;
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"set HIDType to %d", mHIDType);
        return 0;
      } else if (m_eStatus == HID_DEV_CREATING) {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"A device was being created, wait for it to be created then destroy it ");
        while(m_eStatus != HID_DEV_CREATED) {
            MM_Timer_Sleep(2);
        }
        destroy_uhid_device();
      } else if(m_eStatus == HID_DEV_CREATED) {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "Destroying existing device to prepare for a new one");
        destroy_uhid_device();
      }else if (m_eStatus == HID_DEV_DESTROYING) {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"A device was being destroyed, wait for it to be destroyed ");
