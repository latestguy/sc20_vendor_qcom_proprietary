...
m_pHIDInjector(NULL)
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCInputReceiver:constructor");
...
UIBCInputReceiver::~UIBCInputReceiver()
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCInputReceiver:destructor");
        ...
        case FETCH_DATA_SIGNAL:
          {
            if (m_pAttachVMCB)
            {
              ...
              {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"Attaching to VM succeeded");
              }
              else
              {
...
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCInputReceiver:init");
...
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCInputReceiver:Start");
  Init();
...
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCInputReceiver:Stop");
...
void UIBCInputReceiver::CloseDataSource()
{
  ...
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCInputReceiver:releasing signals");
...
boolean UIBCInputReceiver::ParseAndInjectInput()
{
  ...
  {
    {
       MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH, "UIBC Packet Dump Header %x %x",m_UIBCPacket[m_packetReadIndex],m_UIBCPacket[m_packetReadIndex+1]);
    }
    int32 cachedReadIndex = m_packetReadIndex;
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"UIBCInputReceiver:ParseNinjectInput before parsing readIndex %d",m_packetReadIndex);
  //Sanity Check
    if (((m_UIBCPacket[m_packetReadIndex]&0xE0)>>5) != UIBC_WFD_VERSION)
  {
    ...
    if (size > availableSize)
    {
      ...
      MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM,"UIBCInputReceiver:ParseNinjectInput less data %d is available for total size %d",availableSize, size);
      m_packetReadIndex -=2;
      return FALSE;
    }
    m_packetReadIndex+=2;
  if (bIsTSPresent == TRUE)
  {
    ...
    {
       MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM, "******UIBC Packet Dump START********");
       for(int index=0;index<size;index++)
       {
         MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM, "UIBC Packet %d byte - %x", index, m_UIBCPacket[cachedReadIndex+index]);
       }
       MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM, "******UIBC Packet Dump END********");
    }
  ...
  default:
    ...
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"UIBCInputReceiver:ParseNinjectInput after parsing readIndex %d",m_packetReadIndex);
  } // end while
  return TRUE;
}
