...
UIBCPacketTransmitter::UIBCPacketTransmitter( uint16 sourcePort, uint32 sourceIPaddr, uint32 negotiated_height, uint32  negotiated_width )
  :m_nSourcePort(sourcePort),
   ...
   m_pUIBCBuffer(NULL)
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:constructor");
  init();
}

UIBCPacketTransmitter::~UIBCPacketTransmitter()
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:destructor");
  release();
}
...
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:start");
  init();
...
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:stop");
...
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:sendEvent");
...
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:worker thread started");
    ...
    if ( 0 == MM_SignalQ_Wait( m_pSignalQ, (void **) &pEvent ) )
    {
      switch( *pEvent )
      {
        case THREAD_CONNECT_SIGNAL:
        {
          ...
          {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:THREAD_CONNECT_SIGNAL received");
            ...
            if( connectStatus == 0 )
            {
              ...
              {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM, "UIBCPacketTransmitter:connection success: releasing timer");
        ...
        {
          MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:THREAD_EXIT_SIGNAL received");
          isRunning = FALSE;
          break;
        }
        ...
        {
          MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:THREAD_SEND_EVENT_SIGNAL received");
          processEventsList();
          break;
        }
        ...
        {
          MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:THREAD_STOP_SEND_SIGNAL received");
  ...
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:Exiting UIBC thread");
  m_eUIBCState = UIBC_NO_INIT;
...
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:connectTimerHandler");
...
int32 UIBCPacketTransmitter::makeConnection()
{
   ...
   MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:make connection with port %hu, IP %u", m_nSourcePort, m_nSourceIPaddr);
   ...
   if ( 0 != connect(static_cast<int>(m_nSocket), (struct sockaddr*)&tAddr.sa, static_cast<socklen_t>(sizeof(addr))) )
   {
     ...
     {
       MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, " UIBCPacketTransmitter:making select call");
       ...
       MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, " UIBCPacketTransmitter:select return status %d",selectStatus);

       if(FD_ISSET(m_nSocket, &fdWriteSet))
       {
         MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM, " UIBCPacketTransmitter:socket is written");
         int errorNum = 0;
         ...
         if(errorNum == 0)
         {
           MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, " UIBCPacketTransmitter:socket connection established");
           connectStatus = 0;
         }
         else
         {
   ...
   {
     MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, " UIBCPacketTransmitter:socket connect SUCCEEDED");
   }
  ...
  while(qmm_ListIsEmpty(&m_hEventsList) != 1)
  {
    ...
    if(pLink != NULL)
    {
      ...
      if(packetLength > 0)
      {
        ...
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:sendPacket:sent %d bytes", numBytesSent);
      }
      else
      {
...
int32 UIBCPacketTransmitter::sendPacket(uint8* pUIBCPacket, int32 packetLength )
{
  ...
  nBytesSent = send(m_nSocket, pUIBCPacket, packetLength, MSG_NOSIGNAL);
...
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:init");
...
int32 UIBCPacketTransmitter::createSocket()
{
  ...
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:createSocket");
...
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:release");

  if( m_pUIBCThread != NULL )
  {
    MM_Signal_Set(m_pExitSignal);

    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:Waiting for UIBC thread exit");
    int exitCode;
    MM_Thread_Join(m_pUIBCThread, &exitCode);

    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:Releasing UIBC thread");
  ...
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:releasing signals");
  ...
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:release: done");
