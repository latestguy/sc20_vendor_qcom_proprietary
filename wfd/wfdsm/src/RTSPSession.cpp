...
void cback::openCallback(rtspApiMesg &mesg)
{
    ...
    if( mesg.wfd.contentProtection.getValid() == TRUE) {
        if( PRIMARY_SINK == pSM->pMyDevice->getDeviceType() || SECONDARY_SINK == pSM->pMyDevice->getDeviceType()) {
            ...
            // This API shall take longer for Sink since it waits for
            // TCP listen to start on the HDCP port
...
void cback::setupCallback(rtspApiMesg &mesg)
{
    ...
    if(SOURCE != pSM->pMyDevice->getDeviceType())
    {
      if(pSM->pMyDevice->pMMCapability->pCapability != NULL)
      {
          ...
          if (mesg.wfd.contentProtection.getValid())
          {
              MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,
                          "CP valid for session");
...
void* RTSPSession::rtspClientLoopFunc(void *c)
{
    ...
    setThreadName("RTSP_Client");
    MM_MSG_PRIO2(MM_GENERAL,MM_PRIO_ERROR,"RTSP client has %s policy and prio "\
    "%d ",getSchedPolicyName(),androidGetThreadPriority(gettid()));
    setTimerSlackValue(0);
    client->eventLoop();
