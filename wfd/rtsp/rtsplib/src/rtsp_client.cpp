...
void rtspClient::setupClientSocket(string ipaddr)
{
    ...
    {
        int uRetryCount = MIN_RETRY_COUNT; // 50 * 200 Milli Secs = 10000 Millisecs / 10 secs
        int nRetVal;
        int retryCount=0;
        nRetVal = PargeCfgForIntValueForKey((char *)WFD_CFG_FILE, (char *)"RetryCount",
                                      (int*)(&retryCount));
        if(nRetVal==0 && (retryCount>=MIN_RETRY_COUNT))
        {
            uRetryCount = retryCount;
        }
        unsigned long startTime, currTime, diffTime, maxWaitTime;
        maxWaitTime = uRetryCount * RETRY_CONNECT_SLEEP_TIME;
        ...
        for (;;)
        {
            ...
            else if(diffTime > maxWaitTime)
            {
                MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_ERROR,
                    "rtsp connect failed after waiting for %ld ms (MAX = %ld ms)",
                    diffTime, maxWaitTime);
                break;
            }
